...
class SSHKeyService {
    ...
    /**
     * Prepare a temporary key file for SSH -i usage
     * Writes decrypted private key to temp file with 0600 permissions
     *
     * @param int $keyId The SSH key ID
     * @return string|null Path to temp key file, or null if key not found
     */
    public static function prepareTempKeyFile(int $keyId): ?string {
        $key = self::getKey($keyId, true);
        ...
        $tempDir = sys_get_temp_dir();
        $tempKeyFile = $tempDir . '/ssh_key_' . uniqid() . '_' . time();

        // Write private key
        file_put_contents($tempKeyFile, $key['private_key']);
    ...
    /**
     * Securely clean up a temp key file
     * Overwrites with zeros before deletion for security
     *
     * @param string $keyFile Path to temp key file
     */
    public static function cleanupTempKeyFile(string $keyFile): void {
