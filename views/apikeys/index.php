<div class="container py-4" data-assist-page="apikeys" data-assist-purpose="Create and manage API keys for external integrations">
                ...
                <tbody>
                    ...
                    <tr class="<?= (!$key['is_active'] || $key['is_expired']) ? 'table-secondary' : '' ?>">
                        <td>
                            ...
                            <br><small class="text-muted"><?= h($key['description']) ?></small>
                            <?php endif; ?>
                        </td>
                        <td>
                            <code class="text-muted"><?= h($key['token_preview']) ?></code>
                            <button class="btn btn-sm btn-link p-0 ms-1" onclick="regenerateToken(<?= $key['id'] ?>, '<?= h($key['name'], ENT_QUOTES) ?>')" title="Regenerate token">
                                <i class="bi bi-arrow-clockwise"></i>
                            </button>
                        </td>
                        <td>
                            <span title="<?= h($key['member_email'] ?? '') ?>">
                                <?= h($key['member_username'] ?? '') ?>
                            </span>
                            <?php if ($isAdmin && $key['created_by_member_id'] != $key['member_id']): ?>
                            <br><small class="text-muted">by <?= h($key['created_by_username'] ?? '') ?></small>
                            <?php endif; ?>
                        </td>
                        <td>
                            ...
                            <span class="badge bg-info"><?= h($scope) ?></span>
                            <?php endforeach;
                        ...
                        <td>
                            <div class="btn-group btn-group-sm">
                                <button type="button" class="btn btn-outline-success" onclick="showUseKeyModal(<?= $key['id'] ?>, '<?= h($key['name'], ENT_QUOTES) ?>', '<?= h($key['token'], ENT_QUOTES) ?>')" title="Show usage">
                                ...
                                <button type="button" class="btn btn-outline-danger" onclick="deleteKey(<?= $key['id'] ?>, '<?= h($key['name'], ENT_QUOTES) ?>')" title="Delete">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="border rounded p-3" style="max-height: 200px; overflow-y: auto;">
                            ...
                            <div class="form-check">
                                <input class="form-check-input scope-checkbox" type="checkbox" value="<?= h($scope) ?>" id="scope_<?= md5($scope) ?>">
                ...
                <form id="editKeyForm">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="border rounded p-3" style="max-height: 200px; overflow-y: auto;">
                            <?php foreach ($availableScopes as $scope => $desc): ?>
                            <div class="form-check">
                                <input class="form-check-input edit-scope-checkbox" type="checkbox" value="<?= h($scope) ?>" id="edit_scope_<?= md5($scope) ?>">
...
const workspaceSlug = '<?= h($_SESSION['workspace_slug'] ?? 'default') ?>';
