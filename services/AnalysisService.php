...
class AnalysisService {
            ...
            if (!empty($board['aiagents_id'])) {
                ...
                if ($agent && $agent->id) {
                    ...
                    if ($agent->provider === 'ollama') {
                        ...
                        // Get API key from agent's connections_id
                        if (!empty($agent->connections_id)) {
                            $keyData = $this->getBoardAnthropicKey($agent->connections_id);
                            if ($keyData) {
    ...
    /**
     * Get API key and model from board's selected connections record
     *
     * @param int $keyId The connections.id from board's aidev_anthropic_key_id or agent's connections_id
    ...
    private function getBoardAnthropicKey(int $keyId): ?array {
        try {
            ...
            if (!$key->id || $key->connector_type !== 'anthropic' || empty($key->access_token)) {
                $this->logger->warning('API key not found or not an Anthropic connection', [
                    'member_id' => $this->memberId,
                    'key_id' => $keyId
                ]);
                return null;
            ...
            // Decrypt the API key
            $decryptedKey = EncryptionService::decrypt($key->access_token);

            // Read model from metadata_json
