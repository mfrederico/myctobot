...
BASH;
    ...
    private function generateMcpJson(object $job): string {
        ...
        // Try to get cloud ID from atlassian token
        $token = Bean::findOne('atlassiantoken', '(member_id = ? OR is_shared = 1) ORDER BY updated_at DESC LIMIT 1', [$memberId]);
        if ($token) {
    ...
    private function generateEnvSh(object $job): string {
        ...
        if ($job->repoconnections_id) {
            ...
            if ($repo && $repo->aiagents_id) {
                $agent = Bean::load('aiagents', $repo->aiagents_id);
                if ($agent && $agent->anthropickeys_id) {
                    require_once __DIR__ . '/../services/AnthropicKeyService.php';
                    $key = \app\services\AnthropicKeyService::getKey($agent->anthropickeys_id);
