...
php scripts/story-build-orchestrator.php --tenant=footest4 --dry-run --verbose

# Process one story and exit
php scripts/story-build-orchestrator.php --tenant=footest4 --verbose --once

# Run continuously (daemon mode)
php scripts/story-build-orchestrator.php --tenant=footest4 --verbose

# Run with parallel processing (2 stories at a time)
php scripts/story-build-orchestrator.php --tenant=footest4 --verbose --max-concurrent=2
```
...
| Option | Description |
|--------|-------------|
| `--tenant=<name>` | **Required.** Tenant slug (e.g., `footest4`, `gwt`) |
...
2. **Spawns tmux sessions** - Each story gets its own tmux session named:
   ```
   story-{tenant}-{storyId}-{sanitized-issue-key}
   ```
...
```bash
php scripts/local-aidev-full.php --issue=mfrederico/myctobot#99 --tenant=footest4 --print
```
...
- Individual job logs: `log/{tenant}.log`
...
*/5 * * * * cd /path/to/myctobot && php scripts/story-build-orchestrator.php --tenant=footest4 --once >> log/orchestrator.log 2>&1
```

Or run in a screen/tmux session:
```bash
screen -dmS orchestrator php scripts/story-build-orchestrator.php --tenant=footest4 --verbose
```
