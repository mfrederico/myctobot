                ...
                <div class="card-body p-0">
                    <table class="table table-sm mb-0">
                        <tbody>
                            ...
                            <tr>
                                <td><?= htmlspecialchars(\$rel->name ?? \$rel->title ?? 'ID: ' . \$rel->id) ?></td>
                                <td class="text-end">
                    ...
                    <form method="POST" action="/<?=$ctx->beanName?>/<?php echo '<?= $isNew ? \'store\' : \'edit/\' . $bean->id ?>'; ?>">
                        ...
                        <?php echo '<?php if (!empty($csrf) && is_array($csrf)): ?>'; ?>
                            ...
                            <?php echo '<?php foreach ($csrf as $csrfName => $csrfValue): ?>'; ?>
                                ...
                                <input type="hidden" name="<?php echo '<?= htmlspecialchars($csrfName) ?>'; ?>" value="<?php echo '<?= htmlspecialchars($csrfValue) ?>'; ?>">
