...
class Studiodev extends BaseControls\Control {
    ...
    public function index() {
        ...
        // Get GitHub connections (legacy table + unified connections)
        $gitHubConnections = Bean::find('githubtokens', ' member_id = ? ', [$this->member->id]);
        if (empty($gitHubConnections)) {
            $gitHubConnections = Bean::find('connections', ' connector_type = ? AND (member_id = ? OR shared = 1)', ['github', $this->member->id]);
        }

        // Get Jira/Atlassian connections
        $atlassianConnections = Bean::find('connections', ' connector_type = ? AND member_id = ? ', ['atlassian', $this->member->id]);
