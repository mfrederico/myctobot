...
if (isset($options['help']) || empty($options['issue'])) {
    ...
    echo "                  e.g., '{\"host\":\"127.0.0.1\",\"user\":\"youruser\",\"port\":22}'\n";
...
require_once $baseDir . '/services/SSHKeyService.php';
...
// Prepare SSH key file if workstation has an sshkey_id
$sshKeyFile = null;
if ($workstationConfig && !empty($workstationConfig['sshkey_id'])) {
    $sshKeyFile = SSHKeyService::prepareTempKeyFile((int) $workstationConfig['sshkey_id']);
    if ($sshKeyFile) {
        echo "Using SSH key ID {$workstationConfig['sshkey_id']} → {$sshKeyFile}\n";
    } else {
        echo "Warning: SSH key ID {$workstationConfig['sshkey_id']} not found, using default keys\n";
    }
}
...
if ($provider === 'jira' && !$skipJira) {
    $atlassianTokenCheck = Bean::findOne('connections', 'connector_type = ? AND (member_id = ? OR shared = 1)', ['atlassian', $memberId]);
    if ($atlassianTokenCheck) {
        $cloudId = $atlassianTokenCheck->external_eid;
    } else {
...
if ($provider === 'github') {
    ...
    try {
        ...
        $githubConn = Bean::findOne('connections', 'connector_type = ? AND (member_id = ? OR shared = 1) ORDER BY id ASC', ['github', $memberId]);
        if (!$githubConn || empty($githubConn->access_token)) {
            echo "Error: No GitHub token found for member {$memberId}\n";
            exit(1);
        }
        $githubToken = EncryptionService::decrypt($githubConn->access_token);
...
if ($isPipelineJob) {
    ...
    // Try to get GitHub token from unified connections table (might be needed for some MCP servers)
    $githubConn = Bean::findOne('connections', 'connector_type = ? ORDER BY id ASC', ['github']);
    if ($githubConn && !empty($githubConn->access_token)) {
        try {
            $githubToken = EncryptionService::decrypt($githubConn->access_token);
        } catch (Exception $e) {
...
// Try repo's linked connection first
if (empty($githubToken) && !empty($repoBean->connections_id)) {
    $linkedConn = Bean::load('connections', (int)$repoBean->connections_id);
    if ($linkedConn->id && !empty($linkedConn->access_token)) {
        try {
            $githubToken = EncryptionService::decrypt($linkedConn->access_token);
            if (!$isPipelineJob) {
                echo "  GitHub Token (from linked connection #{$linkedConn->id}): ****" . substr($githubToken, -4) . "\n";
            }
        } catch (Exception $e) {
            if (!$isPipelineJob) {
                echo "  Warning: Could not decrypt linked connection GitHub token\n";
            }
        }
    }
}

// Fall back to unified connections table if no token yet
if (empty($githubToken)) {
    ...
    if ($githubConn && !empty($githubConn->access_token)) {
        try {
            $githubToken = EncryptionService::decrypt($githubConn->access_token);
            if (!$isPipelineJob) {
                echo "  GitHub Token (from connections): ****" . substr($githubToken, -4) . "\n";
            }
        } catch (Exception $e) {
            if (!$isPipelineJob) {
                echo "  Warning: Could not decrypt connections GitHub token\n";
            }
        }
    }
}
...
if (!$isPipelineJob) {
    ...
    try {
        $atlassianToken = Bean::findOne('connections', 'connector_type = ? AND (member_id = ? OR shared = 1) AND external_eid = ?', ['atlassian', $memberId, $cloudId]);
        if ($atlassianToken) {
            $jiraEmail = $atlassianToken->email ?? '';
            $siteUrl = $atlassianToken->external_url ?? '';
            if ($siteUrl) {
    ...
    if (empty($linkedServers)) {
        ...
        foreach ($linkedServers as $server) {
            ...
            if ($serverType === 'sse') {
                ...
                $serverUrl = $substitute($server->url);

                // Auto-populate builtin HTTP servers that have empty URLs
                if (empty($serverUrl) && $server->is_builtin) {
                    $wsBaseUrl = \app\services\SiteConfig::getWorkspaceUrl($mcpWorkspace);
                    $apiToken = $member->api_token ?? '';

                    if ($serverName === 'pipelines') {
                        $serverUrl = $wsBaseUrl . '/mcp/pipelines';
                        $serverHeaders['Authorization'] = "Bearer {$apiToken}";
                        $serverHeaders['X-Workspace'] = $mcpWorkspace;
                        echo "      → Auto-configured pipelines URL: {$serverUrl}\n";
                    } elseif ($serverName === 'jira') {
                        $serverUrl = $wsBaseUrl . '/mcp/jira';
                        $serverHeaders['Authorization'] = 'Basic ' . base64_encode("{$memberId}:{$cloudId}");
                        $serverHeaders['X-MCP-Agent-Name'] = $agentConfig['name'] ?? 'AI Developer';
                        echo "      → Auto-configured jira URL: {$serverUrl}\n";
                    }
                }

                // For jobs-type servers, always override auth to include job_uid
                // The jobs endpoint needs Basic auth with memberId:jobUid to identify the job
                if (strpos($serverUrl, '/mcp/jobs') !== false && !empty($mcpJobUidForAuth)) {
                    $serverHeaders['Authorization'] = 'Basic ' . base64_encode("{$memberId}:{$mcpJobUidForAuth}");
                    echo "      → Auto-configured jobs auth (member:{$memberId}, job:{$mcpJobUidForAuth})\n";
                }
...
if ($agentConfig && (!empty($agentConfig['provider_config']['use_ollama']) || $agentConfig['provider'] === 'ollama')) {
    if ($agentConfig['provider'] === 'ollama') {
        $ollamaHost = $agentConfig['provider_config']['base_url'] ?? 'http://localhost:11434';
        $ollamaModel = $agentConfig['provider_config']['model'] ?? 'qwen3-coder';
    } else {
...
// Export SSH key file path so do_cleanup can clean it up
$sshKeyFileExport = $sshKeyFile ? "\nSSH_KEY_FILE=" . escapeshellarg($sshKeyFile) . "\n" : "\nSSH_KEY_FILE=\n";
$envScript .= $sshKeyFileExport;
...
do_cleanup() {
    ...
    # Securely clean up temp SSH key file
    if [[ -n "$SSH_KEY_FILE" && -f "$SSH_KEY_FILE" ]]; then
        echo "Cleaning up SSH key file..."
        dd if=/dev/zero of="$SSH_KEY_FILE" bs=$(stat -c%s "$SSH_KEY_FILE" 2>/dev/null || echo 1) count=1 2>/dev/null
        rm -f "$SSH_KEY_FILE"
    fi
...
$modelFlag = '';
$ollamaSshEnvSingle = ''; // For single-quoted SSH args (print mode)
$ollamaSshEnvDouble = ''; // For double-quoted SSH args (interactive mode heredoc)
if (!empty($ollamaModel)) {
    $modelFlag = "--model {$ollamaModel}";
    // Build env exports for SSH commands (remote workstation needs these too)
    // Single-quote context: no escaping needed inside single-quoted SSH arg
    $ollamaSshEnvSingle = "export ANTHROPIC_AUTH_TOKEN=ollama && export ANTHROPIC_API_KEY= && export ANTHROPIC_BASE_URL={$ollamaHost} && ";
    // Double-quote context (heredoc with \\\$ escaping): escape inner quotes
    $ollamaSshEnvDouble = "export ANTHROPIC_AUTH_TOKEN=ollama && export ANTHROPIC_API_KEY= && export ANTHROPIC_BASE_URL={$ollamaHost} && ";
}
...
$sshSuffix = '';
$sshScriptFile = '';
// Build SSH key flag for ssh (-i keyfile) and scp (-i keyfile) commands
$sshKeyFlag = $sshKeyFile ? "-i " . escapeshellarg($sshKeyFile) . " " : '';
$scpKeyFlag = $sshKeyFlag; // Same flag works for both ssh and scp
if ($workstationConfig) {
...
if ($usePrintMode) {
    ...
    if ($workstationConfig && $sshScriptFile) {
        ...
        $claudeCmd = "ssh {$sshKeyFlag}-p {$wsPort} {$wsUser}@{$wsHost} 'mkdir -p {$remoteWorkDir}/.claude' && " .
            "scp {$scpKeyFlag}-P {$wsPort} prompt.txt {$wsUser}@{$wsHost}:{$remoteWorkDir}/prompt.txt && " .
...
BASH;
    ...
    // Only kill session if NOT in resident mode
    if (!$residentMode) {
        $envScript .= <<<'BASH'

# Run full cleanup before killing session (tmux kill-session prevents trap from firing)
do_cleanup

# Kill tmux session now that cleanup is done
tmux kill-session -t "$AOE_SESSION_NAME" 2>/dev/null
exit 0
BASH;
    } else {
        $envScript .= <<<'BASH'
...
ssh {$sshKeyFlag}-p {$wsPort} {$wsUser}@{$wsHost} "mkdir -p {$remoteWorkDir}/.claude"
...
SSHSCRIPT;
        ...
        $scriptCommand = "script -q session.log -c 'claude --dangerously-skip-permissions {$modelFlag} \"\$(cat {$workDir}/prompt.txt)\"'";
    }
...
# Kill tmux session after cleanup has finished
tmux kill-session -t "$AOE_SESSION_NAME" 2>/dev/null
exit 0
BASH;
    } else {
...
if ($useJobExecutor) {
    ...
    // Use workspace URL for callback so it routes to the correct workspace
