...
class Pipelines extends BaseControls\Control {
    ...
    public function index($params = []) {
        ...
        $allTags = [];
        foreach ($pipelineList as $pl) {
            foreach ($pl['tags'] as $tag) {
    ...
    public function store($params = []) {
        ...
        // Parse tags
        $tagsRaw = trim($this->getParam('tags', ''));
        $tags = $tagsRaw ? implode(',', array_unique(array_filter(array_map(function($t) {
    ...
    public function edit($params = []) {
        ...
        $shopifyConnections = Bean::find('connections',
            ' connector_type = ? AND enabled = 1 AND (member_id = ? OR shared = 1) ORDER BY external_name ASC ',
            ['shopify', $this->member->id]
        );

        // Build webhook URL for this pipeline using workspace URL
        $workspaceSlug = $_SESSION['workspace_slug'] ?? 'default';
        $baseUrl = \app\services\SiteConfig::getWorkspaceUrl();
        $webhookUrl = "{$baseUrl}/pipein/{$pipeline->slug}";
        $mcpToolsUrl = "{$baseUrl}/pipelines/mcp/tools";
    ...
    public function update($params = []) {
        ...
        $tagsRaw = $this->getParam('tags', null);
        if ($tagsRaw !== null) {
            $tagsRaw = trim($tagsRaw);
            $pipeline->tags = $tagsRaw ? implode(',', array_unique(array_filter(array_map(function($t) {
                return strtolower(trim($t));
            }, explode(',', $tagsRaw))))) : '';
        }
    ...
    /**
     * Test Shopify Liquid evaluation endpoint (AJAX)
     */
    public function testliquideval($params = []) {
        if (!$this->requireLogin()) return;
        if (!$this->validateCSRFHeader()) {
            return;
        }
        ...
        $liquidCode = trim($input['code'] ?? '');
        $url = trim($input['url'] ?? '');

        if (empty($connectionId)) {
            Flight::jsonError('Shopify connection required', 400);
            return;
        }

        if (empty($liquidCode)) {
            Flight::jsonError('Liquid code required', 400);
            return;
        }

        // Security: limit code length
        if (strlen($liquidCode) > 50000) {
            Flight::jsonError('Liquid code too long (max 50KB)', 400);
            return;
        }
        ...
        try {
            ...
            $conn = \app\services\ConnectionsService::getConnection($connectionId);
            if (!$conn || $conn->connector_type !== 'shopify') {
                Flight::jsonError('Shopify connection not found', 404);
                return;
            }

            // Use ShopifyClient to upload to dev theme and generate preview
            ...
            try {
                ...
                Flight::json([
                    'success' => false,
                    'error' => 'ShopifyClient error: ' . $e->getMessage()
                ]);
                return;
            }

            // Get existing themes and use first unpublished or dev theme
            try {
                $themes = $client->getThemes();
                error_log("[testliquideval] Got " . count($themes) . " themes");
            } catch (\Exception $e) {
                error_log("[testliquideval] getThemes failed: " . $e->getMessage());
                Flight::json([
                    'success' => false,
                    'error' => 'Failed to get themes: ' . $e->getMessage()
                ]);
                return;
            }

            // Find a dev theme (role=development)
            $targetTheme = null;
            foreach ($themes as $theme) {
                if ($theme['role'] === 'development') {
                    $targetTheme = $theme;
                    error_log("[testliquideval] Using existing dev theme: " . ($theme['name'] ?? $theme['id']));
                    break;
                }
            }

            // If no dev theme found, look for one by name (might be unpublished)
            if (!$targetTheme) {
                foreach ($themes as $theme) {
                    $name = strtolower($theme['name'] ?? '');
                    if (strpos($name, 'preview') !== false || strpos($name, 'mycto') !== false || strpos($name, 'dev') !== false) {
                        $targetTheme = $theme;
                        error_log("[testliquideval] Using dev theme by name: " . ($theme['name'] ?? $theme['id']));
                        break;
                    }
                }
            }

            // If still no dev theme, create one automatically
            if (!$targetTheme) {
                error_log("[testliquideval] No dev theme found, creating one automatically");
                try {
                    // Find main/live theme to duplicate from
                    $sourceThemeId = null;
                    foreach ($themes as $theme) {
                        if ($theme['role'] === 'main') {
                            $sourceThemeId = (int)$theme['id'];
                            break;
                        }
                    }

                    $createdTheme = $client->createDevelopmentTheme('MyCTO Preview Dev', $sourceThemeId);
                    $targetTheme = $createdTheme;
                    error_log("[testliquideval] Created dev theme: " . ($createdTheme['name'] ?? $createdTheme['id']));
                } catch (\Exception $e) {
                    error_log("[testliquideval] Failed to create dev theme: " . $e->getMessage());
                    Flight::json([
                        'success' => false,
                        'error' => 'No dev theme found and failed to create one: ' . $e->getMessage()
                    ]);
                    return;
                }
            }

            $numericThemeId = (int)$targetTheme['id'];
            error_log("[testliquideval] Using theme ID: " . $numericThemeId . " (from " . print_r($targetTheme, true) . ")");

            // Upload Liquid code as a snippet
            $snippetKey = 'snippets/preview-' . time() . '.liquid';
            error_log("[testliquideval] Uploading snippet: " . $snippetKey);

            try {
                $uploadResult = $client->updateThemeAsset($numericThemeId, $snippetKey, $liquidCode);
                error_log("[testliquideval] Upload result: " . ($uploadResult ? 'success' : 'failed'));
            } catch (\Exception $e) {
                error_log("[testliquideval] Upload exception: " . $e->getMessage());
                Flight::json([
                    'success' => false,
                    'error' => 'Upload failed: ' . $e->getMessage(),
                    'debug' => [
                        'theme_id' => $numericThemeId,
                        'theme_data' => $targetTheme,
            ...
            if (!$uploadResult) {
                error_log("[testliquideval] Upload returned false");
                Flight::json([
                    'success' => false,
                    'error' => 'Failed to upload Liquid code to theme'
                ]);
                return;
            }
            ...
            $previewUrl = "https://{$shop}{$url}?preview_theme_id={$numericThemeId}";
            error_log("[testliquideval] Preview URL: " . $previewUrl);
            ...
            error_log("[testliquideval] Response sent");
    ...
    public function runsync($params = []) {
        ...
        // Execute synchronously via dispatcher (engine or in-process fallback)
        $workspaceSlug = $_SESSION['workspace_slug'] ?? $this->workspaceSlug ?? 'default';
        \app\services\PipelineDispatcher::dispatch($runId, $workspaceSlug, true);
    ...
    /**
     * Render a multi-step wizard for pipeline input
     *
     * GET /pipelines/wizard/{run_id}?token={input_token}&embed=1
     *
     * Displays a wizard-style progress bar and form for multi-step pipelines.
     * Supports embed mode for iframe integration.
     */
    public function wizard($params = []) {
        $runId = (int) ($this->opId() ?? $this->getParam('run_id') ?? 0);
        $token = $this->getParam('token');
        $embedMode = (bool) $this->getParam('embed');
        ...
        $authenticated = false;
        if (!empty($token)) {
            // Find any step with this token (current or past)
            $stepWithToken = Bean::findOne('pipelinestepruns',
        ...
        if (!$authenticated) {
            $this->render('pipelines/wizard', [
                'error' => 'Input token required or login to continue',
                'embed' => $embedMode
            ]);
            return;
        }

        // Get pipeline and all its steps
        $pipeline = $run->pipelines;
        // Fallback: explicitly load pipeline if relation didn't work
        if (!$pipeline || !$pipeline->id) {
            $pipeline = Bean::load('pipelines', $run->pipelines_id);
        }
        ...
        $pipelineSteps = [];
        if ($pipeline && $pipeline->id) {
            $pipelineSteps = Bean::find('pipelinesteps',
                ' pipelines_id = ? ORDER BY sequence ',
                [$pipeline->id]
            );
        }

        // Filter to only await_input steps for progress tracking
        // Note: await_input can be step_type='await_input' OR step_type='wait' with wait_type='await_input'
        $awaitInputSteps = [];
        foreach ($pipelineSteps as $step) {
            $isAwaitInput = ($step->step_type === 'await_input');
            if (!$isAwaitInput && $step->step_type === 'wait') {
                $stepConfig = json_decode($step->config_json ?: '{}', true);
                $isAwaitInput = (($stepConfig['wait_type'] ?? '') === 'await_input');
            }
            ...
            if ($isAwaitInput) {
                $awaitInputSteps[] = [
                    'id' => $step->id,
                    'step_name' => $step->step_name,
                    'label' => $step->label ?: ucwords(str_replace('_', ' ', $step->step_name)),
                    'sequence' => $step->sequence
                ];
            }
        }

        // Get completed await_input step runs
        $completedStepRuns = Bean::find('pipelinestepruns',
            ' pipelineruns_id = ? AND status = ? ',
            [$run->id, 'completed']
        );

        // Count how many await_input steps have been completed
        $completedAwaitStepNames = [];
        foreach ($completedStepRuns as $stepRun) {
            // Look up the step type
            $step = Bean::findOne('pipelinesteps',
                ' pipelines_id = ? AND step_name = ? ',
                [$pipeline->id, $stepRun->step_name]
            );
            if ($step) {
                $isAwaitInput = ($step->step_type === 'await_input');
                if (!$isAwaitInput && $step->step_type === 'wait') {
                ...
                if ($isAwaitInput) {
                    $completedAwaitStepNames[] = $stepRun->step_name;
                }
            }
        }

        $totalSteps = count($awaitInputSteps);
        $completedSteps = count($completedAwaitStepNames);
        $currentStepNum = $completedSteps + 1;
        ...
        $progress = [
            ...
            'steps' => $awaitInputSteps,
            'completed_names' => $completedAwaitStepNames
        ];

        // Check if pipeline is complete (no awaiting step and all done)
        if (!$awaitingStep) {
            $isComplete = in_array($run->status, ['completed', 'failed']);
            $this->render('pipelines/wizard', [
                'run_id' => $run->id,
                'run_uid' => $run->run_uid,
                'token' => $token ?? '',
                ...
                'embed' => $embedMode,
                'error' => $isComplete ? null : 'Pipeline is processing. Please wait...'
            ]);
            return;
        }
        ...
        if ($awaitingStep->awaiting_input_timeout_at && strtotime($awaitingStep->awaiting_input_timeout_at) < time()) {
            $this->render('pipelines/wizard', [
                'error' => 'This input window has expired',
                'embed' => $embedMode
            ]);
            return;
        }

        // Get schema and context for current step
        $schema = json_decode($awaitingStep->awaiting_input_schema_json ?: '{}', true);
        $prompt = $awaitingStep->awaiting_input_prompt ?: 'Please provide the requested information';
        $context = json_decode($run->context_json ?: '{}', true);

        // Use the step's actual token for submission
        $actualToken = $awaitingStep->awaiting_input_token;
        ...
        $this->render('pipelines/wizard', [
            ...
            'pipeline_name' => $pipeline->name ?? 'Pipeline',
            'step_name' => $awaitingStep->step_name,
            'step_label' => $awaitInputSteps[$currentStepNum - 1]['label'] ?? $awaitingStep->step_name,
            'timeout_at' => $awaitingStep->awaiting_input_timeout_at,
            'progress' => $progress,
            'embed' => $embedMode,
            'completed' => false
     ...
     *
     * POST /pipelines/wizardsubmit/{run_id}
     *
     * Processes form submission and returns JSON for wizard navigation.
     */
    public function wizardsubmit($params = []) {
        $runId = (int) ($this->opId() ?? $this->getParam('run_id') ?? 0);
        $token = $this->getParam('token');
        ...
        if (empty($inputData)) {
            Flight::json(['success' => false, 'error' => 'No input data provided'], 400);
            return;
        }

        if (!$run || !$run->id) {
            Flight::json(['success' => false, 'error' => 'Run not found'], 404);
            return;
        }
        ...
        if (!$awaitingStep) {
            Flight::json(['success' => false, 'error' => 'Pipeline is not awaiting input'], 400);
            return;
        }
        ...
        $actualToken = $awaitingStep->awaiting_input_token;
        $currentStepName = $awaitingStep->step_name;

        // Resume the pipeline with the input
        require_once __DIR__ . '/../services/PipelineExecutor.php';

        try {
            $this->logger->info('wizardsubmit resuming pipeline', [
                'run_id' => $run->id,
            ...
            // Check if there's another await_input step
            $nextAwaitingStep = Bean::findOne('pipelinestepruns',
                ' pipelineruns_id = ? AND awaiting_input = 1 ',
                [$run->id]
            );
            ...
            // Get all pipeline steps and filter for await_input types
            $allSteps = [];
            if ($pipeline && $pipeline->id) {
                $allSteps = Bean::find('pipelinesteps',
                    ' pipelines_id = ? ORDER BY sequence ',
            ...
            // Filter to await_input steps (step_type='await_input' OR wait with wait_type='await_input')
            $awaitInputStepNames = [];
            foreach ($allSteps as $step) {
                $isAwaitInput = ($step->step_type === 'await_input');
                if (!$isAwaitInput && $step->step_type === 'wait') {
                ...
                if ($isAwaitInput) {
                    $awaitInputStepNames[] = $step->step_name;
                }
            }
            $totalSteps = count($awaitInputStepNames);
            ...
            $completedCount = 0;
            foreach ($completedStepRuns as $stepRun) {
                if (in_array($stepRun->step_name, $awaitInputStepNames)) {
                    $completedCount++;
                }
            }

            $isComplete = in_array($run->status, ['completed', 'failed']);
            $hasMoreSteps = $nextAwaitingStep !== null;
            ...
            Flight::json([
                ...
                'run_uid' => $run->run_uid,
                'status' => $run->status,
                'completed_step' => $currentStepName,
                'has_next_step' => $hasMoreSteps,
                'pipeline_complete' => $isComplete,
                'progress' => [
                    'current' => $completedCount + ($hasMoreSteps ? 1 : 0),
                    'total' => $totalSteps,
                    'percentage' => $totalSteps > 0 ? round(($completedCount / $totalSteps) * 100) : 100
                ],
                'next_token' => $nextAwaitingStep ? $nextAwaitingStep->awaiting_input_token : null
            ]);

        } catch (\Exception $e) {
            $this->logger->error('wizardsubmit error', [
                'run_id' => $run->id,
                'error' => $e->getMessage()
            ]);
            Flight::json([
    ...
    public function export($params = []) {
        ...
        $exportData = [
            'version' => '1.0',
            'exported_at' => date('c'),
            'source_workspace' => $_SESSION['workspace_slug'] ?? 'default',
            'pipeline' => [
                'name' => $pipeline->name,
                'slug' => $pipeline->slug,
                'description' => $pipeline->description,
                ...
                'trigger_type' => $pipeline->trigger_type,
                'trigger_config_json' => $this->ensureObject(json_decode($pipeline->trigger_config_json ?: '{}', true)),
                'default_context_json' => $this->ensureObject(json_decode($pipeline->default_context_json ?: '{}', true)),
                'input_schema_json' => $this->ensureObject(json_decode($pipeline->input_schema_json ?: '{}', true)),
                'expose_as_tool' => (bool) $pipeline->expose_as_tool,
                'is_active' => (bool) $pipeline->is_active
            ],
            'steps' => [],
        ...
        foreach ($steps as $step) {
            $config = $this->ensureObject(json_decode($step->config_json ?: '{}', true));
            ...
            $stepData = [
                ...
                'config_json' => $config,
                'input_source' => $step->input_source,
                'input_config_json' => $this->ensureObject(json_decode($step->input_config_json ?: '{}', true)),
                'condition_json' => $this->ensureObject(json_decode($step->condition_json ?: '{}', true)),
                'output_mappings_json' => $this->ensureObject(json_decode($step->output_mappings_json ?: '{}', true)),
                'on_success' => $step->on_success,
                'on_failure' => $step->on_failure,
                'timeout_seconds' => (int) $step->timeout_seconds,
                'retry_count' => (int) $step->retry_count,
            ...
            $exportData['steps'][] = $stepData;

            // Extract dependencies based on step type
            $this->extractDependencies($step->step_type, $config, $exportData['dependencies']);
        }

        // Remove duplicates from dependencies
        foreach ($exportData['dependencies'] as $key => $values) {
            $exportData['dependencies'][$key] = array_values(array_unique($values));
        }

        // Set headers for file download
        $filename = $pipeline->slug . '-pipeline.json';
    ...
    public function import($params = []) {
        ...
        if (!isset($_FILES['pipeline_file']) || $_FILES['pipeline_file']['error'] !== UPLOAD_ERR_OK) {
            Flight::jsonError('No file uploaded or upload error', 400);
            return;
        }

        $uploadedFile = $_FILES['pipeline_file'];

        // Validate file type
        $mimeType = mime_content_type($uploadedFile['tmp_name']);
        if (!in_array($mimeType, ['application/json', 'text/plain', 'text/json'])) {
            Flight::jsonError('Invalid file type. Please upload a JSON file.', 400);
            return;
        }
        ...
        if (json_last_error() !== JSON_ERROR_NONE) {
            Flight::jsonError('Invalid JSON: ' . json_last_error_msg(), 400);
            return;
        }

        // Validate structure
        if (!isset($data['version']) || !isset($data['pipeline']) || !isset($data['steps'])) {
            Flight::jsonError('Invalid pipeline export format. Missing required fields.', 400);
            return;
        }
        ...
        if (empty($pipelineData['name']) || empty($pipelineData['slug'])) {
            Flight::jsonError('Invalid pipeline data. Name and slug are required.', 400);
            return;
        }

        // Generate unique slug if collision
        $baseSlug = $this->sanitizeSlug($pipelineData['slug']);
        $slug = $this->generateUniqueSlug($baseSlug);

        // Check for missing dependencies
        $warnings = $this->checkDependencies($dependencies, $stepsData);

        // Create the pipeline
        $pipeline = Bean::dispense('pipelines');
        $pipeline->member = $this->member;
        $pipeline->slug = $slug;
        $pipeline->name = $pipelineData['name'] . ($slug !== $baseSlug ? ' (Imported)' : '');
        $pipeline->description = $pipelineData['description'] ?? '';
        $pipeline->columns_json = json_encode($pipelineData['columns_json'] ?? ['Start', 'Execute', 'Validate', 'Complete']);
        $pipeline->trigger_type = $pipelineData['trigger_type'] ?? 'manual';
        $pipeline->trigger_config_json = json_encode($this->ensureObject($pipelineData['trigger_config_json'] ?? []));
        $pipeline->default_context_json = json_encode($this->ensureObject($pipelineData['default_context_json'] ?? []));
        $pipeline->input_schema_json = json_encode($this->ensureObject($pipelineData['input_schema_json'] ?? []));
        $pipeline->expose_as_tool = ($pipelineData['expose_as_tool'] ?? false) ? 1 : 0;
        $pipeline->is_active = ($pipelineData['is_active'] ?? true) ? 1 : 0;
        $pipeline->is_system = 0; // Imported pipelines are never system pipelines
        $pipeline->run_count = 0;
        $pipeline->created_at = date('Y-m-d H:i:s');
        $pipeline->updated_at = date('Y-m-d H:i:s');
        ...
        foreach ($stepsData as $stepData) {
            $step = Bean::dispense('pipelinesteps');
            $step->pipelines = $pipeline;
            $step->step_name = $stepData['step_name'] ?? 'step_' . $stepCount;
            $step->label = $stepData['label'] ?? $step->step_name;
            $step->row = (int) ($stepData['row'] ?? 0);
            $step->col = (int) ($stepData['col'] ?? 0);
            $step->step_type = $stepData['step_type'] ?? 'direct_exec';
            $step->config_json = json_encode($this->ensureObject($stepData['config_json'] ?? []));
            $step->input_source = $stepData['input_source'] ?? 'context';
            $step->input_config_json = json_encode($this->ensureObject($stepData['input_config_json'] ?? []));
            $step->condition_json = json_encode($this->ensureObject($stepData['condition_json'] ?? []));
            $step->output_mappings_json = json_encode($this->ensureObject($stepData['output_mappings_json'] ?? []));
    ...
    /**
     * Extract dependency references from step config
     */
    private function extractDependencies(string $stepType, array $config, array &$dependencies): void {
        switch ($stepType) {
            case 'ai_agent':
                if (!empty($config['agent_slug'])) {
                    $dependencies['ai_agents'][] = $config['agent_slug'];
                } elseif (!empty($config['agent_id'])) {
                    // Try to resolve agent_id to slug
                    $agent = Bean::load('aiagents', (int) $config['agent_id']);
                    if ($agent && $agent->id) {
                        $dependencies['ai_agents'][] = $agent->slug ?? ('agent_' . $agent->id);
                    }
                }
                break;
            ...
            case 'mcp_call':
                if (!empty($config['server'])) {
                    $dependencies['mcp_servers'][] = $config['server'];
                } elseif (!empty($config['server_id'])) {
                    $server = Bean::load('mcpservers', (int) $config['server_id']);
                    if ($server && $server->id) {
                        $dependencies['mcp_servers'][] = $server->name ?? ('server_' . $server->id);
                    }
                }
                break;
            ...
            case 'script':
                if (!empty($config['repo_id'])) {
                    $repo = Bean::load('repoconnections', (int) $config['repo_id']);
                    if ($repo && $repo->id) {
                        $dependencies['repo_connections'][] = $repo->slug ?? $repo->repo_full_name ?? ('repo_' . $repo->id);
                    }
                }
                break;
            ...
            case 'shopify_graphql':
                if (!empty($config['connection_id'])) {
                    // connection_id might be a variable like {context.shop_connection_id}
                    if (!preg_match('/^\{/', $config['connection_id'])) {
                        $conn = Bean::load('connections', (int) $config['connection_id']);
                        if ($conn && $conn->id && $conn->connector_type === 'shopify') {
                            $dependencies['shopify_connections'][] = $conn->external_name ?? ('shop_' . $conn->id);
                        }
                    }
                }
                break;
            ...
            case 'direct_exec':
                if (!empty($config['workstation_id'])) {
                    $runner = Bean::load('runners', (int) $config['workstation_id']);
                    if ($runner && $runner->id) {
                        $dependencies['workstations'][] = $runner->name ?? ('workstation_' . $runner->id);
                    }
                }
                break;
        }
     ...
     */
    private function checkDependencies(array $dependencies, array $stepsData): array {
        $warnings = [];

        // Check AI agents
        if (!empty($dependencies['ai_agents'])) {
            foreach ($dependencies['ai_agents'] as $agentSlug) {
                $agent = Bean::findOne('aiagents', 'slug = ? OR name = ?', [$agentSlug, $agentSlug]);
                if (!$agent) {
                    $warnings[] = "AI Agent not found: {$agentSlug}";
                }
            }
        }

        // Check MCP servers
        if (!empty($dependencies['mcp_servers'])) {
            foreach ($dependencies['mcp_servers'] as $serverName) {
                $server = Bean::findOne('mcpservers', 'name = ?', [$serverName]);
                if (!$server) {
                    $warnings[] = "MCP Server not found: {$serverName}";
                }
            }
        }

        // Check repo connections
        if (!empty($dependencies['repo_connections'])) {
            foreach ($dependencies['repo_connections'] as $repoRef) {
                $repo = Bean::findOne('repoconnections', 'slug = ? OR repo_full_name = ?', [$repoRef, $repoRef]);
                if (!$repo) {
                    $warnings[] = "Repository connection not found: {$repoRef}";
                }
            }
        }

        // Check Shopify connections
        if (!empty($dependencies['shopify_connections'])) {
            foreach ($dependencies['shopify_connections'] as $shopName) {
                $conn = Bean::findOne('connections', 'connector_type = ? AND external_name = ?', ['shopify', $shopName]);
                if (!$conn) {
                    $warnings[] = "Shopify connection not found: {$shopName}";
                }
            }
        }

        // Check workstations/runners
        if (!empty($dependencies['workstations'])) {
            foreach ($dependencies['workstations'] as $workstationName) {
                $runner = Bean::findOne('runners', 'name = ?', [$workstationName]);
                if (!$runner) {
                    $warnings[] = "Workstation/runner not found: {$workstationName}";
                }
            }
        }
    ...
    /**
     * Ensure a value is an object for JSON encoding
     * Empty arrays become empty objects to output {} instead of []
     */
    ...
    /**
     * Start the pipeline engine for this workspace
     *
     * POST /pipelines/startengine
     */
    public function startengine($params = []) {
        if (!$this->requireLogin()) return;
        if (!$this->validateCSRF()) {
            Flight::jsonError('Invalid request', 403);
            return;
        ...
        try {
            $manager = new \app\services\ServiceManager($this->logger);

            // Create engine service record if it doesn't exist
            $engine = $manager->createEngine($workspace, $this->member->id ?? null);

            // Start it
            $result = $manager->start($engine->id);

            Flight::jsonSuccess($result, 'Pipeline engine started');
        } catch (\Exception $e) {
            $this->logger->error('Failed to start pipeline engine', [
                'workspace' => $workspace,
                'error' => $e->getMessage()
            ]);
            Flight::jsonError('Failed to start engine: ' . $e->getMessage(), 500);
        }
    }

    /**
     * Stop the pipeline engine for this workspace
     *
     * POST /pipelines/stopengine
     */
    public function stopengine($params = []) {
        if (!$this->requireLogin()) return;
        if (!$this->validateCSRF()) {
            Flight::jsonError('Invalid request', 403);
            return;
        ...
        try {
            $engine = Bean::findOne('tenantapps',
                ' workspace = ? AND service_type = ? ',
                [$workspace, 'engine']
            );

            if (!$engine) {
                Flight::jsonError('No engine found for this workspace', 404);
                return;
            }

            $manager = new \app\services\ServiceManager($this->logger);
            $result = $manager->stop($engine->id);

            Flight::jsonSuccess($result, 'Pipeline engine stopped');
        } catch (\Exception $e) {
            $this->logger->error('Failed to stop pipeline engine', [
                'workspace' => $workspace,
                'error' => $e->getMessage()
            ]);
            Flight::jsonError('Failed to stop engine: ' . $e->getMessage(), 500);
        }
    }

    /**
     * Get pipeline steps list with input schema (AJAX)
     *
     * GET /pipelines/pipelinestepslist/{pipelineId}
     * Returns steps and input_schema for dynamic UI (e.g., schedule_task entry step dropdown)
     */
    public function pipelinestepslist($params = []) {
        if (!$this->requireLogin()) return;

        $pipelineId = (int) ($this->opId() ?? $this->getParam('id') ?? 0);
        if (!$pipelineId) {
            Flight::json(['success' => false, 'error' => 'Pipeline ID required']);
            return;
        }

        $pipeline = Bean::load('pipelines', $pipelineId);
        if (!$pipeline || !$pipeline->id) {
            Flight::json(['success' => false, 'error' => 'Pipeline not found']);
            return;
        }

        $steps = Bean::find('pipelinesteps', ' pipelines_id = ? ORDER BY row ASC, col ASC ', [$pipelineId]);
        $stepList = [];
        foreach ($steps as $s) {
            $stepList[] = [
        ...
        $inputSchema = json_decode($pipeline->input_schema_json ?: '{}', true) ?: (object)[];
    ...
    /**
     * Get pipeline engine status for this workspace
     *
     * GET /pipelines/enginestatus
     */
    public function enginestatus($params = []) {
        if (!$this->requireLogin()) return;
        ...
        try {
            $manager = new \app\services\ServiceManager($this->logger);
            $status = $manager->status($engine->id);
            $status['exists'] = true;
    ...
    // ========================================================================
    // Template Gallery (absorbed from Orchestration)
    // ========================================================================

    /**
     * Template categories (from Orchestration controller)
     */
    ...
    /**
     * Schedule types with UI metadata (from Schedules controller)
     */
    ...
    /**
     * Overlap policies (from Schedules controller)
     */
    ...
    /**
     * Common timezones (from Schedules controller)
     */
    ...
    /**
     * AJAX: Template gallery HTML fragment
     *
     * GET /pipelines/templategallery?category=ecommerce
     */
    public function templategallery($params = []) {
        if (!$this->requireLogin()) return;
        ...
        if ($category && isset(self::TEMPLATE_CATEGORIES[$category])) {
            $sql .= ' AND category = ? ';
            $bindings[] = $category;
        }
        ...
        $templates = Bean::find('studiotemplates', $sql, $bindings);

        $templateList = [];
        foreach ($templates as $t) {
            $templateList[] = $this->formatTemplateData($t);
        }

        // Render as AJAX HTML fragment (no layout)
        ob_start();
        $tplTemplates = $templateList;
        $tplCategories = self::TEMPLATE_CATEGORIES;
        $tplSelectedCategory = $category;
        include __DIR__ . '/../views/pipelines/template_gallery.php';
        $html = ob_get_clean();
    ...
    /**
     * Template wizard - full page view
     *
     * GET /pipelines/templatewizard/123
     */
    public function templatewizard($params = []) {
        if (!$this->requireLogin()) return;
        ...
        // Default wizard config for custom orchestrations
        if (!$wizardConfig) {
            $wizardConfig = [
                'steps' => [
                    [
        ...
        $this->viewData['title'] = $template ? "Create: {$template->name}" : 'Create from Template';
        $this->viewData['template'] = $template ? $this->formatTemplateData($template) : null;
        $this->viewData['wizardConfig'] = $wizardConfig;
        $this->viewData['templateId'] = $templateId;

        $this->render('pipelines/template_wizard', $this->viewData);
    }

    /**
     * AJAX: Save wizard step progress
     *
     * POST /pipelines/templatewizardstep
     */
    public function templatewizardstep($params = []) {
        if (!$this->requireLogin()) return;
        ...
        try {
            ...
            if ($projectId) {
                $project = Bean::load('studioprojects', (int) $projectId);
            } else {
    ...
    /**
     * AJAX: Finalize template wizard and generate pipeline
     *
     * POST /pipelines/templatefinalize
     */
    public function templatefinalize($params = []) {
        if (!$this->requireLogin()) return;
        ...
        try {
            ...
            if (!$projectName && $templateId) {
                $template = Bean::load('studiotemplates', (int) $templateId);
                $projectName = $template->name . ' - ' . date('M j, Y');
            }
            $project->name = $projectName ?: 'Untitled Pipeline';

            // Generate slug
            $slug = strtolower($project->name);
            $slug = preg_replace('/[^a-z0-9]+/', '-', $slug);
            $slug = trim($slug, '-');
            $originalSlug = $slug;
            $counter = 1;
            ...
            // Generate the pipeline via the generator service
            if ($project->studiotemplates_id) {
                $template = Bean::load('studiotemplates', (int) $project->studiotemplates_id);
                if ($template->id) {
                    require_once __DIR__ . '/../services/StudioPipelineGenerator.php';
                    $generator = new \app\services\StudioPipelineGenerator($project->member_id);
                    $variables = json_decode($project->variables_json ?: '{}', true);
                    $genResult = $generator->generate($template, $variables, $project->name);

                    if ($genResult['success']) {
                        $project->pipelines_id = $genResult['pipeline_id'];
            ...
            Flight::jsonSuccess([
                ...
                'pipeline_id' => null,
                'step_count' => 0
            ], 'Project created (no template pipeline generated)');
    ...
    // ========================================================================
    // Schedules Tab (absorbed from Schedules)
    // ========================================================================

    /**
     * AJAX: Schedule list HTML fragment
     *
     * GET /pipelines/schedulestab
     */
    public function schedulestab($params = []) {
        if (!$this->requireLogin()) return;

        require_once __DIR__ . '/../services/SchedulerService.php';

        $schedules = Bean::findAll('scheduledrecurring', ' ORDER BY next_run_at ASC ');
        ...
        foreach ($schedules as $s) {
            ...
            // Get next 5 run times for preview
            $nextRuns = \app\services\SchedulerService::previewNextRuns(
                $s->schedule_type,
                $config,
                $s->timezone,
                5
            ...
            $scheduleList[] = [
                ...
                'pipeline_slug' => $pipeline ? $pipeline->slug : null,
                'schedule_type' => $s->schedule_type,
                'schedule_type_info' => self::SCHEDULE_TYPES[$s->schedule_type] ?? null,
                'schedule_config' => $config,
                'timezone' => $s->timezone,
                'is_active' => (bool) $s->is_active,
                'max_concurrent' => (int) $s->max_concurrent,
                'on_overlap' => $s->on_overlap,
                'next_run_at' => $s->next_run_at,
                'last_run_at' => $s->last_run_at,
                'run_count' => (int) $s->run_count,
                'current_running' => (int) $s->current_running,
                'next_runs' => $nextRuns,
                'created_at' => $s->created_at
            ];
        }

        // Get all active pipelines for the create dropdown
        $allPipelines = Bean::findAll('pipelines', ' is_active = 1 ORDER BY name ASC ');

        // Render as AJAX HTML fragment (no layout)
        ob_start();
        $tplSchedules = $scheduleList;
        $tplPipelines = $allPipelines;
        $tplScheduleTypes = self::SCHEDULE_TYPES;
        $tplOverlapPolicies = self::OVERLAP_POLICIES;
        $tplTimezones = self::TIMEZONES;
        include __DIR__ . '/../views/pipelines/schedule_tab.php';
        $html = ob_get_clean();
    ...
    /**
     * Edit schedule form - full page view
     *
     * GET /pipelines/scheduleedit/123
     */
    public function scheduleedit($params = []) {
        ...
        $this->viewData['title'] = $schedule ? 'Edit Schedule: ' . $schedule->name : 'Create Schedule';
        $this->viewData['schedule'] = $schedule;
        $this->viewData['scheduleConfig'] = $config;
        $this->viewData['nextRuns'] = $nextRuns;
        $this->viewData['pipelines'] = $allPipelines;
        ...
        $this->render('pipelines/schedule_edit', $this->viewData);
    }

    /**
     * Store/update a schedule
     *
     * POST /pipelines/schedulestore
     */
    public function schedulestore($params = []) {
        if (!$this->requireLogin()) return;
        if (!$this->validateCSRF()) {
            $this->flash('error', 'Invalid request');
            Flight::redirect('/pipelines?tab=schedules');
        ...
        if (empty($name)) {
            ...
            Flight::redirect($id ? '/pipelines/scheduleedit/' . $id : '/pipelines/scheduleedit');
            return;
        }
        ...
        $config = $this->buildScheduleConfigFromParams($scheduleType);
        ...
        if ($id) {
            ...
            $schedule->next_run_at = \app\services\SchedulerService::calculateNextRun(
                $scheduleType,
                $config,
                $timezone
            );
            ...
            Flight::redirect('/pipelines/scheduleedit/' . $id);
        } else {
            // Create new schedule via SchedulerService
            $errors = \app\services\SchedulerService::validate([
                'pipeline_id' => $pipelineId,
            ...
            if (!empty($errors)) {
                ...
                Flight::redirect('/pipelines/scheduleedit');
                return;
            }

            $scheduleId = \app\services\SchedulerService::create([
                'pipeline_id' => $pipelineId,
                'name' => $name,
                'description' => trim($this->getParam('description', '')),
                'schedule_type' => $scheduleType,
            ...
            Flight::redirect('/pipelines/scheduleedit/' . $scheduleId);
        }
    }
     ...
     *
     * POST /pipelines/scheduledelete/123
     */
     ...
     *
     * POST /pipelines/scheduletoggle/123
     */
    public function scheduletoggle($params = []) {
        if (!$this->requireLogin()) return;
    ...
    /**
     * Get next run preview (AJAX) for schedule form
     *
     * GET /pipelines/schedulepreview
     */
    public function schedulepreview($params = []) {
        if (!$this->requireLogin()) return;
    ...
    // ========================================================================
    // Private Helpers for Templates & Schedules
    // ========================================================================

    /**
     * Format template bean for view consumption
     */
    private function formatTemplateData($template): array {
        return [
            'id' => $template->id,
            'name' => $template->name,
            'slug' => $template->slug,
            'category' => $template->category,
            'category_info' => self::TEMPLATE_CATEGORIES[$template->category] ?? null,
    ...
    /**
     * Build schedule config from POST/GET parameters
     */
    private function buildScheduleConfigFromParams(string $type): array {
        $config = [];
