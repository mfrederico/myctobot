<div class="container py-4"
     data-assist-page="<?= isset($runner) ? 'workstations/edit' : 'workstations/create' ?>"
     data-assist-purpose="<?= isset($runner) ? 'Configure an existing workstation for running AI agents and pipeline jobs' : 'Add a new workstation (server) that can execute AI agents and pipeline jobs' ?>"
     data-assist-context='{"entity":"workstation","isNew":<?= isset($runner) ? 'false' : 'true' ?>}'>
            ...
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0">
                        ...
                        <?= isset($runner) ? 'Edit Workstation: ' . h($runner['name']) : 'Add Workstation' ?>
                    </h5>
                </div>
                <div class="card-body">
                    <form method="POST" action="/workstations/<?= isset($runner) ? 'edit/' . $runner['id'] : 'create' ?>" id="workstationForm"
                          data-assist-form="workstation-config"
                          data-assist-purpose="Configure workstation connection settings, execution mode, and capabilities">
                        ...
                        <div class="mb-4">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="name" name="name"
                                       value="<?= h($runner['name'] ?? '') ?>" required
                                       placeholder="e.g., Local Dev or Production Workstation 1"
                                       data-assist-field="name"
                                       data-assist-required="true"
                                       data-assist-hint="Required. A descriptive name for this workstation. Examples: 'Local Dev', 'Production Server 1', 'GPU Workstation'."
                                       aria-describedby="nameHelp">
                                <div id="nameHelp" class="form-text visually-hidden">Enter a descriptive name for this workstation</div>
                            </div>

                            <div class="mb-3">
                                <label for="description" class="form-label">Description</label>
                                <textarea class="form-control" id="description" name="description" rows="2"
                                          placeholder="Optional description of this workstation's purpose"
                                          data-assist-field="description"
                                          data-assist-hint="Describe this workstation's purpose and any special capabilities. Helps team members choose the right workstation for their tasks."
                                          aria-describedby="descHelp"><?= h($runner['description'] ?? '') ?></textarea>
                                <div id="descHelp" class="form-text visually-hidden">Describe this workstation's purpose</div>
                            </div>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <div class="row"
                                 data-assist-field="execution_mode"
                                 data-assist-hint="SSH+Tmux is recommended - uses your Claude subscription interactively. HTTP API mode uses API credits and requires a runner server.">
                                <div class="col-md-6">
                        ...
                        <div class="mb-4" id="sshSettings">
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" class="form-control" id="host" name="host"
                                           value="<?= h($runner['host'] ?? 'localhost') ?>" required
                                           placeholder="e.g., localhost or 10.0.0.5"
                                           data-assist-field="host"
                                           data-assist-required="true"
                                           data-assist-hint="Required. The hostname or IP address. Use 'localhost' for local development, or an IP/hostname for remote servers."
                                ...
                                <div class="col-md-3 mb-3">
                                    ...
                                    <input type="text" class="form-control" id="ssh_user" name="ssh_user"
                                           value="<?= h($runner['ssh_user'] ?? 'claudeuser') ?>"
                                           placeholder="claudeuser"
                                           data-assist-field="ssh_user"
                                           data-assist-required="true"
                                           data-assist-hint="Required. The Unix user for SSH login. 'claudeuser' is recommended for dedicated AI workstations."
                                </div>
                                <div class="col-md-3 mb-3">
                                    ...
                                    <input type="number" class="form-control" id="ssh_port" name="ssh_port"
                                           ...
                                           data-assist-field="ssh_port"
                                           data-assist-hint="SSH port number. Default is 22. Only change if the server uses a non-standard port.">
                                </div>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="sshkey_id" name="sshkey_id"
                                        data-assist-field="sshkey_id"
                                        data-assist-hint="Select an SSH private key for authentication. Use system default for local dev, or upload a key in SSH Keys section for remote servers.">
                                    ...
                                    <?php if (!empty($sshKeys)): ?>
                                        ...
                                        <option value="<?= $key['id'] ?>" <?= ($runner['sshkey_id'] ?? '') == $key['id'] ? 'selected' : '' ?>>
                                            <?= h($key['name']) ?> (<?= $key['key_type'] ?>) - <?= substr($key['fingerprint'], 0, 20) ?>...
                                        </option>
                                        <?php endforeach; ?>
                                    <?php endif; ?>
                                </select>
                                <div class="form-text">
                                    Select an SSH key from your <a href="/workstations#sshkeys">SSH Keys</a> or use system default.
                                </div>
                            </div>
                                    ...
                                    <div class="row align-items-end">
                                        <div class="col-md-8 mb-2 mb-md-0">
                                            ...
                                            <input type="text" class="form-control form-control-sm" id="job_executor_url"
                                                   value="<?= h(\app\services\JobExecutorConfig::getUrl()) ?>" placeholder="https://jobs.myctobot.ai">
                                        </div>
                                        <div class="col-md-4">
                            ...
                            <div class="row">
                                <div class="col-md-8 mb-3">
                                    ...
                                    <input type="text" class="form-control" id="http_host"
                                           value="<?= h($runner['host'] ?? '') ?>"
                                           placeholder="e.g., 10.0.0.5 or runner1.example.com"
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select class="form-select" id="runner_type" name="runner_type"
                                            data-assist-field="runner_type"
                                            data-assist-hint="Categorizes this workstation. General = most tasks, Playwright = browser testing, Database = DB operations, Full = all capabilities.">
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="number" class="form-control" id="max_concurrent_jobs" name="max_concurrent_jobs"
                                           ...
                                           data-assist-field="max_concurrent_jobs"
                                           data-assist-hint="Maximum parallel jobs. Start with 1-2 and increase based on CPU/memory. Each Claude session uses significant resources.">
                                    <div class="form-text">How many jobs can run simultaneously on this workstation.</div>
                                </div>
                            </div>

                            <div class="mb-3"
                                 data-assist-field="capabilities"
                                 data-assist-hint="Select the tools and runtimes installed on this workstation. Agents can require specific capabilities when being assigned to a workstation.">
                                ...
                                <div class="col-md-6">
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" id="is_active" name="is_active" value="1"
                                               ...
                                               data-assist-hint="When disabled, the workstation won't receive new jobs. Use this for maintenance without deleting the workstation.">
                                ...
                                <div class="col-md-6">
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" id="is_default" name="is_default" value="1"
                                               ...
                                               data-assist-field="is_default"
                                               data-assist-hint="Public workstations are available to all team members. Private workstations require explicit assignment to use.">
...
async function testSSH() {
    ...
    try {
        ...
        const response = await fetch('/workstations/test/<?= $runner['id'] ?>', {
            method: 'POST',
            body: formData
        });
...
async function runHelloWorld() {
    ...
    try {
        ...
        const response = await fetch('/workstations/helloworld/<?= $runner['id'] ?>', {
            method: 'POST',
            body: formData
        });
        const data = await response.json();
...
async function runTestWithAgent() {
    ...
    try {
        ...
        const response = await fetch('/workstations/testwithagent/<?= $runner['id'] ?>', {
            method: 'POST',
            body: formData
        });
        const data = await response.json();
...
async function pollWsTestStatus(originalBtnHtml) {
    ...
    try {
        const response = await fetch('/workstations/teststatus/' + wsCurrentTestId);
        const data = await response.json();
        ...
        try {
            await fetch('/workstations/teststatus/' + wsCurrentTestId + '?send_exit=1');
        } catch (e) {
...
async function testJobExecutor() {
    ...
    try {
        ...
        const response = await fetch('/workstations/testjobexecutor/<?= $runner['id'] ?>', {
            method: 'POST',
            body: formData
        });
        const data = await response.json();
...
async function runDiagnostic() {
    ...
    try {
        ...
        const response = await fetch('/workstations/diagnose/<?= $runner['id'] ?>', {
            method: 'POST',
            body: formData
        });
        const data = await response.json();
