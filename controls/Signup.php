...
class Signup extends BaseControls\Control {
    ...
    public function index() {
        ...
        if (!WorkspaceResolver::isDefault()) {
            // Redirect to the main site's signup page, preserving any workspace param
            $baseUrl = \app\services\SiteConfig::getBaseUrl();
            $subdomain = WorkspaceResolver::getSubdomain();
            $query = $subdomain ? '?workspace=' . urlencode($subdomain) : '';
            Flight::redirect("{$baseUrl}/signup{$query}");
            return;
        }
    ...
    public function verify($params) {
        ...
        try {
            ...
            // Redirect to the workspace's own login page
            $workspaceUrl = \app\services\SiteConfig::getWorkspaceUrl($pending->subdomain);
            Flight::redirect("{$workspaceUrl}/login?welcome=1");
