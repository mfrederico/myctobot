...
class GitOperations {
    ...
    public static function ensureRepoCloned(string $workDir, string $issueKey, int $memberId): bool {
        ...
        if (empty($token) && !empty($repo->connections_id)) {
            $linkedConn = \app\Bean::load('connections', (int)$repo->connections_id);
            if ($linkedConn->id && !empty($linkedConn->access_token)) {
                $token = EncryptionService::decrypt($linkedConn->access_token);
            }
        }

        // If no token in repo connection, try unified connections table based on provider
        if (empty($token)) {
            $provider = $repo->provider ?? 'github';
            $conn = \app\Bean::findOne('connections', 'connector_type = ? AND enabled = 1 AND (shared = 1 OR 1=1) ORDER BY shared DESC, id ASC', [$provider]);
