...
class RunnerRouter {
    ...
    public static function getMemberMcpCredentials(int $memberId, ?string $cloudId = null): array {
        ...
        $token = Bean::findOne('connections', 'connector_type = ? AND ' . $whereClause, array_merge(['atlassian'], $params));

        if ($token) {
            // For OAuth tokens, must use Atlassian API gateway URL (not external_url)
            // external_url is for browser links, API calls must go through api.atlassian.com
            $jiraHost = AtlassianAuth::getApiBaseUrl($token->external_eid);
