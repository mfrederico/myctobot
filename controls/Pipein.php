...
class Pipein extends BaseControls\Control {
     ...
     *
     * Authentication:
     * - Pipelines with trigger_type='webhook' and trigger_config.allow_public=true
     *   can be triggered without API key (optional webhook_secret validation)
     * - All other pipelines require API key authentication
     *
    ...
    public function index($params = []) {
        ...
        // Find pipeline by slug FIRST to check if public webhooks are allowed
        ...
        // Check if this pipeline allows public webhooks
        $triggerConfig = json_decode($pipeline->trigger_config_json ?: '{}', true);
        $allowPublic = ($pipeline->trigger_type === 'webhook' && !empty($triggerConfig['allow_public']));
        $memberId = 1; // Default system user for public webhooks

        if ($allowPublic) {
            // Public webhook - validate secret if configured, but no API key needed
            ...
            $this->logger->info("Public pipein webhook for workspace: {$workspace}, pipeline: {$pipelineSlug}", [
                'ip' => Flight::request()->ip
            ]);
        } else {
            // Private webhook - require API key authentication
            ...
            // Still validate webhook secret if configured (additional security layer)
            if (!empty($triggerConfig['webhook_secret'])) {
                if (!$this->validateWebhookSecret($triggerConfig['webhook_secret'])) {
                    $this->errorResponse(401, 'Invalid webhook secret');
                    return;
        ...
        $triggerSource = $allowPublic ? 'webhook:public' : 'webhook';
        ...
        // Merge trigger data into context (so {context.email} etc. work)
