...
abstract class Control {
    ...
    protected function render($template, $data = [], $layout = true) {
        ...
        if ($layout) {
            // Use sidebar layout for logged-in users, top nav for public
            if (Flight::isLoggedIn() && empty($data['hideSidebar'])) {
                $data['useSidebar'] = true;
                Flight::render('layouts/sidebar', $data, 'sidebar_content');
                Flight::render('layouts/topbar', $data, 'header_content');
            } elseif (Flight::isLoggedIn() && !empty($data['hideSidebar'])) {
                $data['useSidebar'] = false;
                Flight::render('layouts/topbar', $data, 'header_content');
                Flight::view()->set('sidebar_content', '');
            } else {
                $data['useSidebar'] = false;
    ...
    protected function sanitize($input, $type = 'string') {
        switch ($type) {
            ...
            case 'html':
                return h($input, ENT_QUOTES, 'UTF-8');
