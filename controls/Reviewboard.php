...
class Reviewboard extends BaseControls\Control {
    ...
    private function getGitHubConfig(): ?array {
        ...
        // Get token from unified connections table
        $conn = Bean::findOne('connections', 'connector_type = ? ORDER BY id ASC', ['github']);
        if (!$conn || !$conn->access_token) {
            return null;
        }

        $accessToken = EncryptionService::decrypt($conn->access_token);
    ...
    public function getbranches() {
        ...
        try {
            ...
            $token = \app\services\EncryptionService::decrypt($conn->access_token);
