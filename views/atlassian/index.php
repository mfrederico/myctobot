<div class="container py-4" data-assist-page="atlassian" data-assist-purpose="Connect and manage Atlassian/Jira account integration">
    <div class="row">
        <div class="col-md-8 offset-md-2">
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    ...
                    <?php $webhooks = $webhooksPerSite[$site->external_eid] ?? []; ?>
                    <div class="p-3 bg-light rounded mb-3">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                <h5 class="mb-1">
                                    <i class="bi bi-building"></i>
                                    <?= h($site->external_name) ?>
                                </h5>
                                <a href="<?= h($site->external_url) ?>" target="_blank" class="text-muted small">
                                    <?= h($site->external_url) ?>
                            ...
                            <div class="btn-group">
                                <button type="button" class="btn btn-outline-primary btn-sm" onclick="refreshToken('<?= $site->external_eid ?>')">
                                    <i class="bi bi-arrow-repeat"></i> Refresh
                                </button>
                                <button type="button" class="btn btn-outline-danger btn-sm" onclick="disconnectSite('<?= $site->external_eid ?>', '<?= h($site->external_name) ?>')">
                        ...
                        <div class="mt-3 pt-3 border-top">
                            <div class="d-flex gap-2 mb-3">
                                <a href="/boards/discover?cloud_uid=<?= urlencode($site->external_eid) ?>" class="btn btn-sm btn-outline-primary">
                                    <i class="bi bi-search"></i> Discover Boards
                                </a>
                                <a href="/boards?cloud_uid=<?= urlencode($site->external_eid) ?>" class="btn btn-sm btn-outline-secondary">
                            ...
                            <div class="table-responsive">
                                <table class="table table-sm table-bordered mb-0 bg-white">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td><code><?= h($webhook['id'] ?? 'N/A') ?></code></td>
                                            <td class="text-break" style="max-width: 200px;">
                                                <small><?= h($webhook['url'] ?? 'N/A') ?></small>
                                            </td>
                                            <td><code><?= h($webhook['jqlFilter'] ?? 'N/A') ?></code></td>
                                            <td>
                                                <?php if (!empty($webhook['events'])): ?>
                                                    <?php foreach ($webhook['events'] as $event): ?>
                                                    <span class="badge bg-secondary me-1"><?= h($event) ?></span>
                                                    <?php endforeach; ?>
                                                <?php else: ?>
                            ...
                            <div class="mt-2">
                                <a href="/atlassian/refreshwebhook?cloud_uid=<?= urlencode($site->external_eid) ?>"
