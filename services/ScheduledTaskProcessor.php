...
class ScheduledTaskProcessor
{
    ...
    private function processExecutePipeline($task, array $payload): array
    {
        ...
        try {
            ...
            // Execute the pipeline via dispatcher (engine or in-process fallback)
            $workspace = TenantAppDatabase::getCurrentWorkspace() ?? 'default';
            PipelineDispatcher::dispatch($runId, $workspace, true);
