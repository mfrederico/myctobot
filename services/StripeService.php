...
class StripeService {
    ...
    public static function getOrCreateCustomer(string $billingEmail, ?string $billingName = null): string {
        ...
        if ($subscription && !empty($subscription->stripe_customer_eid) &&
            strpos($subscription->stripe_customer_eid, 'stub_') !== 0 &&
            strpos($subscription->stripe_customer_eid, 'manual_') !== 0) {

            // Update customer email if different
            try {
                $customer = Customer::retrieve($subscription->stripe_customer_eid);
                if ($customer->email !== $billingEmail) {
                    Customer::update($subscription->stripe_customer_eid, [
                        'email' => $billingEmail,
                ...
                Flight::get('log')->warning('Stripe customer not found, creating new', [
                    'old_customer_id' => $subscription->stripe_customer_eid
                ]);
            }
        ...
        $subscription->stripe_customer_eid = $customer->id;
    ...
    public static function createPortalSession(string $returnUrl): PortalSession {
        ...
        $session = PortalSession::create([
            'customer' => $subscription->stripe_customer_eid,
    ...
    private static function handleSubscriptionUpdate($stripeSubscription): void {
        ...
        $subscription = Bean::findOne('subscription', 'stripe_customer_eid = ?', [$customerId]);
        ...
        $subscription->stripe_subscription_eid = $stripeSubscription->id;
    ...
    private static function handleSubscriptionCanceled($stripeSubscription): void {
        ...
        $subscription->stripe_subscription_eid = null;
        $subscription->cancelled_at = date('Y-m-d H:i:s');
        $subscription->updated_at = date('Y-m-d H:i:s');
    ...
    private static function handlePaymentFailed($invoice): void {
        ...
        $subscription = Bean::findOne('subscription', 'stripe_customer_eid = ?', [$invoice->customer]);
        if ($subscription) {
    ...
    public static function cancelSubscription(): bool {
        ...
        if (!$subscription || empty($subscription->stripe_subscription_eid)) {
            return false;
        }

        try {
            Subscription::update(
                $subscription->stripe_subscription_eid,
