...
### Shard Server (shard.example.com)
```
...
```bash
# Watch shard log
ssh claudeuser@shard.example.com "tail -f /var/www/html/default/myctobot/log/shard-$(date +%Y-%m-%d).log"

# Watch specific job
ssh claudeuser@shard.example.com "tail -f /tmp/aidev-job-{JOB_ID}/session.log"

# List running jobs
ssh claudeuser@shard.example.com "ls -la /tmp/aidev-job-*/"
```
