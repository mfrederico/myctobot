...
async function loadSSHKeys() {
    ...
    try {
        const response = await fetch('/admin/sshkeys');
...
async function generateKey() {
    ...
    try {
        const response = await fetch('/admin/generatesshkey', {
            method: 'POST',
            headers: {
...
async function showPublicKey(keyId) {
    try {
        const response = await fetch(`/admin/getsshkey/${keyId}`);
        const data = await response.json();
...
async function showPrivateKey(keyId) {
    ...
    try {
        const response = await fetch(`/admin/getsshkey/${keyId}?include_private=1`);
        const data = await response.json();
...
async function deleteKey(keyId, keyName) {
    ...
    try {
        const response = await fetch(`/admin/deletesshkey/${keyId}`, { method: 'DELETE' });
        const data = await response.json();
