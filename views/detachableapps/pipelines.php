...
<div class="container py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <p class="text-muted mb-0">Bind pipelines as API endpoints in the detachable app</p>
        </div>
        <div class="d-flex gap-2">
            <a href="/detachableapps/screens/<?= $appId ?>" class="btn btn-outline-secondary">
                <i class="bi bi-arrow-left"></i> Back to Screens
            </a>
            <a href="/detachableapps/download/<?= $appId ?>" class="btn btn-success">
...
function togglePipeline(checkbox) {
    ...
    const endpoint = isBind
        ? '/detachableapps/bindpipeline/' + appId
        : '/detachableapps/unbindpipeline/' + appId;
...
document.querySelectorAll('.pipeline-alias').forEach(input => {
    input.addEventListener('change', function() {
        ...
        fetch('/detachableapps/bindpipeline/' + appId, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded',
                'X-Requested-With': 'XMLHttpRequest'
