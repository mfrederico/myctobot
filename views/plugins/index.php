<div class="container py-4" data-assist-page="plugins" data-assist-purpose="Browse and install plugins from the marketplace">
            ...
            <form method="GET" action="/plugins" id="searchForm" class="row g-3 align-items-end">
                ...
                <div class="col-md-4 col-lg-3">
                    ...
                    <select class="form-select form-select-lg" id="categorySelect" name="category">
                        ...
                        <option value="<?= h($cat['key']) ?>"
                                <?= ($category ?? '') === $cat['key'] ? 'selected' : '' ?>>
                            <?= h($cat['name']) ?> (<?= $cat['count'] ?>)
                        </option>
                        <?php endforeach; ?>
                    </select>
                </div>
    ...
    <div class="mb-3">
        <span class="badge bg-info fs-6">
            <i class="bi bi-search"></i> Results for: "<?= h($query) ?>"
        </span>
        <?php if (!empty($category)): ?>
        <span class="badge bg-secondary fs-6 ms-1">
            Category: <?= h($categories[array_search($category, array_column($categories, 'key'))]['name'] ?? $category) ?>
        </span>
        <?php endif; ?>
    ...
    <div class="card border-warning">
        <div class="card-body text-center py-5">
            ...
            <h4 class="mt-3"><?= h($noResultsSuggestions['message']) ?></h4>
            ...
            <div class="mt-4 text-start" style="max-width: 600px; margin: 0 auto;">
                ...
                <div class="list-group">
                    ...
                    <a href="/plugins/view/<?= h($plugin['slug']) ?>" class="list-group-item list-group-item-action">
                        <strong><?= h($plugin['name']) ?></strong>
                        <span class="badge bg-info ms-2"><?= h($plugin['category']) ?></span>
                    </a>
                    <?php endforeach; ?>
                </div>
                <?php endif; ?>
        ...
        <div class="col">
            <div class="card h-100 plugin-card">
                ...
                <div class="card-body">
                    ...
                    <h5 class="card-title">
                        <a href="/plugins/view/<?= h($plugin['slug']) ?>" class="text-decoration-none">
                            <?= h($plugin['name']) ?>
                        </a>
                    </h5>

                    <p class="card-text text-muted small">
                        <?= h($plugin['short_description'] ?: substr($plugin['description'] ?? '', 0, 100)) ?>
                    ...
                    <div class="mb-2">
                        ...
                        <span class="badge bg-light text-dark"><?= h($tag) ?></span>
                        <?php endforeach; ?>
                ...
                <div class="card-footer bg-transparent">
                    ...
                    <div class="mt-2">
                        <a href="/plugins/view/<?= h($plugin['slug']) ?>"
