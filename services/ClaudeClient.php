...
class ClaudeClient implements LLMClientInterface {
    ...
    /**
     * Get the first available API key from the connections table (connector_type = 'anthropic')
    ...
    private function getKeyFromDatabase(): ?array {
        try {
            ...
            // Get any shared key or first available anthropic connection
            $key = \app\Bean::findOne('connections', ' connector_type = ? AND (shared = 1 OR 1=1) ORDER BY shared DESC, id ASC ', ['anthropic']);
    ...
    public static function isConfigured(): bool {
        // Check database first (priority) - look for anthropic connections
        try {
            $count = \app\Bean::count('connections', ' connector_type = ? ', ['anthropic']);
            if ($count > 0) {
                return true;
            }
        } catch (\Exception $e) {
