<div class="container py-4" data-assist-page="pluginsources" data-assist-purpose="Manage plugin repository sources">
                    ...
                    <tr id="source-row-<?= $source['id'] ?>">
                        <td>
                            <span class="badge bg-secondary">
                                ...
                                ?>
                                <i class="bi <?= $icon ?>"></i>
                                <?= h(ucfirst($source['provider'])) ?>
                            </span>
                        </td>
                        <td>
                            <code><?= h($source['repository_url']) ?></code>
                            <?php if (!empty($source['description'])): ?>
                            <br><small class="text-muted"><?= h($source['description']) ?></small>
                            <?php endif; ?>
                        </td>
                        <td>
                            <span class="text-muted"><?= h(ucfirst($source['source_type'])) ?></span>
                        </td>
                        <td>
                            <?php
                            $statusBadges = [
                            ...
                            <span class="badge <?= $statusClass ?>" id="status-badge-<?= $source['id'] ?>">
                                <?= h(ucfirst($source['validation_status'])) ?>
                            </span>
                            <?php if ($source['validation_status'] === 'error' && !empty($source['validation_error'])): ?>
                            <br><small class="text-danger" id="validation-error-<?= $source['id'] ?>"><?= h($source['validation_error']) ?></small>
                            <?php endif; ?>
                        </td>
                        <td>
                            <?php if (!empty($source['last_validated_at'])): ?>
                            <small class="text-muted" id="last-validated-<?= $source['id'] ?>"><?= h($source['last_validated_at']) ?></small>
                            <?php else: ?>
