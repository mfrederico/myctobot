<div class="container py-4"
     data-assist-page="<?= $pipeline ? 'pipelines/edit' : 'pipelines/create' ?>"
     data-assist-purpose="<?= $pipeline ? 'Edit pipeline configuration, add/modify steps, and configure triggers' : 'Create a new automated workflow pipeline with triggers and steps' ?>"
     data-assist-context='{"entity":"pipeline","isNew":<?= $pipeline ? 'false' : 'true' ?>}'>
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-1">
                    <li class="breadcrumb-item"><a href="/pipelines">Pipelines</a></li>
                    <li class="breadcrumb-item active"><?= $pipeline ? h($pipeline['name']) : 'New Pipeline' ?></li>
                </ol>
            </nav>
            <h1 class="h2 mb-0">
                <i class="bi bi-diagram-3"></i>
                <?= $pipeline ? 'Edit: ' . h($pipeline['name']) : 'Create Pipeline' ?>
        ...
        <div>
            ...
            <a href="/pipelines/export/<?= $pipeline['id'] ?>" class="btn btn-outline-secondary me-2" title="Export pipeline as JSON">
            ...
            <div class="btn-group ms-2" id="engineControls">
                <button class="btn btn-outline-secondary btn-sm" id="engineStatusBtn" onclick="toggleEngine()" title="Pipeline Engine">
                    <i class="bi bi-cpu" id="engineIcon"></i>
                    <span id="engineStatusLabel">Engine</span>
                </button>
            </div>
            <?php endif; ?>
        </div>
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="/pipelines/store"
                  data-assist-form="pipeline-create"
                  data-assist-purpose="Create a new pipeline with name, trigger type, and initial configuration">
                <input type="hidden" name="csrf_token" value="<?= Flight::csrf()->getToken() ?>">

                <div class="row">
                    <div class="col-md-6">
                        <div class="mb-3">
                            <label class="form-label" for="pipeline-name">Name <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" name="name" id="pipeline-name" required
                                   placeholder="My Deploy Pipeline"
                                   data-assist-field="name"
                                   data-assist-required="true"
                                   data-assist-hint="Required. A descriptive name for your pipeline. Use action-oriented names like 'Deploy to Production' or 'Process Customer Orders'."
                                   aria-describedby="pipelineNameHelp">
                            <div id="pipelineNameHelp" class="form-text visually-hidden">Enter a descriptive name</div>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="mb-3">
                            <label class="form-label" for="pipeline-slug">Slug</label>
                            <input type="text" class="form-control" name="slug" id="pipeline-slug"
                                   placeholder="my-deploy-pipeline"
                                   data-assist-field="slug"
                                   data-assist-hint="URL-safe identifier used in webhook URLs and API calls. Leave empty to auto-generate from the name."
                                   aria-describedby="slugHelp">
                            <small id="slugHelp" class="text-muted">URL-safe identifier. Auto-generated if empty.</small>
                        </div>
                    </div>
                </div>

                <div class="mb-3">
                    <label class="form-label" for="pipeline-desc">Description</label>
                    <textarea class="form-control" name="description" id="pipeline-desc" rows="2"
                              placeholder="What does this pipeline do?"
                              data-assist-field="description"
                              data-assist-hint="Describe what this pipeline does, when it runs, and what it accomplishes. Helps team members understand its purpose."
                              aria-describedby="descHelp"></textarea>
                    <div id="descHelp" class="form-text visually-hidden">Describe what the pipeline does</div>
                </div>

                <div class="mb-3">
                    <label class="form-label" for="pipeline-tags">Tags</label>
                    <input type="text" class="form-control" name="tags" id="pipeline-tags"
                           placeholder="e.g. automation, shopify, email"
                           data-assist-field="tags"
                           data-assist-hint="Comma-separated tags for categorizing and filtering pipelines on the list page.">
                    <small class="text-muted">Comma-separated. Used for filtering on the pipelines list.</small>
                </div>
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <select class="form-select" name="trigger_type" id="trigger-type"
                                    data-assist-field="trigger_type"
                                    data-assist-hint="How the pipeline starts: Manual (button click), Webhook (external HTTP call), Cron (scheduled), or HTTP Poll (check URL periodically)."
                                    aria-describedby="triggerHelp">
                                <?php foreach ($triggerTypes as $type => $info): ?>
                                <option value="<?= $type ?>"><?= $info['label'] ?> - <?= $info['description'] ?></option>
                                <?php endforeach; ?>
                            </select>
                            <div id="triggerHelp" class="form-text visually-hidden">Choose how the pipeline is triggered</div>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" name="columns"
                                   ...
                                   data-assist-field="columns"
                                   data-assist-hint="Visual workflow stages shown as columns in the grid. Steps flow left to right. Common patterns: Fetch/Process/Validate/Notify or Setup/Execute/Cleanup."
                                   aria-describedby="columnsHelp">
                            <small id="columnsHelp" class="text-muted">Comma-separated column names</small>
                        </div>
                    </div>
                </div>
    ...
    <div class="card mb-4">
        <div class="card-header d-flex justify-content-between align-items-center" data-bs-toggle="collapse" data-bs-target="#settingsPanel" style="cursor: pointer;">
            <span>
                ...
                <span class="text-muted">— <?= h($pipeline['name']) ?></span>
                <?php if (!empty($pipeline['expose_as_tool'])): ?>
                <span class="badge bg-info ms-2" title="Exposed as MCP tool: myctobot_<?= h($pipeline['slug']) ?>">
        ...
        <div class="collapse" id="settingsPanel">
            <div class="card-body">
                <form id="settingsForm"
                      data-assist-form="pipeline-settings"
                      data-assist-purpose="Modify pipeline configuration including name, trigger, MCP exposure, and column layout">
                    <div class="row">
                        <div class="col-md-4">
                            <div class="mb-3">
                                <label class="form-label">Name</label>
                                <input type="text" class="form-control" name="name" value="<?= h($pipeline['name']) ?>"
                                       data-assist-field="name"
                                       data-assist-required="true"
                                       data-assist-hint="Required. The display name for this pipeline. Changing it updates how it appears in lists and logs.">
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="mb-3">
                                <label class="form-label">Slug</label>
                                <input type="text" class="form-control" name="slug" value="<?= h($pipeline['slug']) ?>"
                                       data-assist-field="slug"
                                       data-assist-hint="URL-safe identifier. Changing this updates webhook URLs and MCP tool names. Existing integrations will need updating.">
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="mb-3">
                                ...
                                <select class="form-select" name="trigger_type"
                                        data-assist-field="trigger_type"
                                        data-assist-hint="Manual = button click, Webhook = external HTTP POST, Cron = scheduled time, HTTP Poll = periodic URL check.">
                                    <?php foreach ($triggerTypes as $type => $info): ?>
                        ...
                        <div class="col-md-8">
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" name="description" rows="2"
                                          data-assist-field="description"
                                          data-assist-hint="Explain what this pipeline does. Good descriptions help team members understand its purpose and when to use it."><?= h($pipeline['description']) ?></textarea>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" name="tags"
                                       value="<?= h($pipeline['tags'] ?? '') ?>"
                                       placeholder="e.g. automation, shopify, email"
                                       data-assist-field="tags"
                                       data-assist-hint="Comma-separated tags for categorizing and filtering pipelines on the list page.">
                                <small class="text-muted">Comma-separated for filtering</small>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" name="columns"
                               value="<?= h(implode(', ', $pipeline['columns'] ?? [])) ?>"
                               placeholder="Stage 1, Stage 2, Stage 3, Stage 4"
                               data-assist-field="columns"
                               data-assist-hint="Define the visual workflow stages. Steps are placed in columns from left to right. Example: Fetch, Process, Validate, Notify.">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <div class="input-group">
                                    <input type="text" class="form-control font-monospace" value="<?= h($webhookUrl) ?>" readonly id="webhookUrl">
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <div class="input-group">
                                    <input type="text" class="form-control font-monospace" value="php scripts/runpipe.php --workspace=<?= $_SESSION['workspace_slug'] ?? 'default' ?> --pipeline=<?= h($pipeline['slug']) ?>" readonly id="cliCommand">
                        ...
                        <div class="card-body">
                            <div class="form-check form-switch mb-3">
                                <input class="form-check-input" type="checkbox" name="expose_as_tool" id="exposeAsTool"
                                       ...
                                       data-assist-field="expose_as_tool"
                                       data-assist-hint="When enabled, AI agents (like Claude) can trigger this pipeline as an MCP tool. The tool name will be myctobot_[slug].">
                            ...
                            <div id="mcpToolSettings" style="<?= $pipeline['expose_as_tool'] ? '' : 'display: none;' ?>">
                                <div class="mb-3">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="text" class="form-control font-monospace" value="<?= h($pipeline['slug']) ?>" readonly>
                                    </div>
                                    <small class="text-muted">AI agents will call this tool as <code>myctobot_<?= h($pipeline['slug']) ?></code></small>
                                </div>

                                <div class="mb-3">
                                    <label class="form-label">MCP Endpoint</label>
                                    <div class="input-group">
                                        <input type="text" class="form-control font-monospace" value="<?= h($mcpToolsUrl) ?>" readonly id="mcpToolsUrl">
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control font-monospace" id="inputSchemaJson" name="input_schema_json" rows="8"
                                              ...
                                              data-assist-field="input_schema_json"
                                              data-assist-hint="JSON Schema defining what parameters the AI can pass when calling this pipeline. Use 'Set from Row 1' button to auto-generate from {context.xxx} variables in your steps."><?= h($schemaJson) ?></textarea>
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" name="is_active" id="isActive" <?= $pipeline['is_active'] ? 'checked' : '' ?>
                                   data-assist-field="is_active"
                                   data-assist-hint="When disabled, the pipeline cannot be triggered via webhook, MCP, or manual run. Use this to temporarily pause a pipeline without deleting it.">
    ...
    <!-- Pipeline Grid -->
    <div class="card" id="pipelineGridCard" data-assist-section="pipeline-grid" data-assist-hint="The main workflow grid where you add and configure pipeline steps">
                        ...
                        <tr>
                            ...
                            <th class="text-center column-header <?= $isParallelCol ? 'column-parallel' : '' ?>"
                                ...
                                <div class="d-flex align-items-center justify-content-center gap-2">
                                    <span class="column-name cursor-pointer" ondblclick="renameColumn(<?= $colIndex ?>, this.closest('th'))" title="Double-click to rename"><?= h($colName) ?></span>
                                    <button type="button"
                            ...
                            <td class="p-2 drop-zone<?= $isParallelCol ? ' parallel-col' : '' ?>" data-row="<?= $row ?>" data-col="<?= $colIndex ?>"
                                ...
                                <?php if (isset($stepGrid[$row][$colIndex])): ?>
                                    ...
                                    <div class="step-cell bg-<?= $step['type_info']['color'] ?? 'secondary' ?>-subtle border border-<?= $step['type_info']['color'] ?? 'secondary' ?> rounded p-2 <?= !$step['is_active'] ? 'opacity-50' : '' ?>"
                                         ...
                                         data-step-name="<?= h($step['step_name']) ?>"
                                         data-step-type="<?= h($step['step_type']) ?>"
                                         draggable="true"
                                        ...
                                        <div class="d-flex align-items-center mb-1">
                                            ...
                                            <strong class="small"><?= h($step['label']) ?></strong>
                                        </div>
                                        <div class="d-flex align-items-center gap-1">
                                            <code class="small text-muted"><?= h($step['step_name']) ?></code>
                                            <span class="badge bg-<?= $step['type_info']['color'] ?? 'secondary' ?>" style="font-size: 0.6rem;"><?= h($step['step_type']) ?></span>
                                        </div>
                                        ...
                                        $inputLabel = match($inputSource) {
                                            'stdin' => '<i class="bi bi-arrow-left"></i> prev',
                                            'getfrom' => '<i class="bi bi-arrow-left"></i> ' . h(json_decode($step['input_config_json'] ?? '{}', true)['step'] ?? '?'),
                                            default => '<i class="bi bi-globe2"></i> ctx'
                                        };
                                        ?>
                                        <div class="mt-1">
                                            <span class="badge bg-light text-dark border" style="font-size: 0.55rem;"><?= $inputLabel ?></span>
                                        </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="stepForm">
                    ...
                    <div class="mb-3">
                        <label class="form-label fw-bold fs-5">What does this step do?</label>
                        <?php
                        // Define primary step types (always visible) and their display properties
                        $primaryTypes = [
                            'direct_exec'     => ['label' => 'Run Command',  'icon' => 'bi-terminal',     'btnColor' => 'dark'],
                            'parser'          => ['label' => 'Transform',    'icon' => 'bi-braces',       'btnColor' => 'secondary'],
                            'ai_agent'        => ['label' => 'Ask AI',       'icon' => 'bi-robot',        'btnColor' => 'primary'],
                            'shopify_graphql' => ['label' => 'Shopify',      'icon' => 'bi-shop',         'btnColor' => 'success'],
                            'webhook_out'     => ['label' => 'Webhook',      'icon' => 'bi-send',         'btnColor' => 'warning'],
                            'mailgun'         => ['label' => 'Send Email',   'icon' => 'bi-envelope',     'btnColor' => 'danger'],
                        ];
                        // All other types go under "More..."
                        $moreTypes = [
                            'mcp_call'            => ['label' => 'MCP Tool',       'icon' => 'bi-plug',              'btnColor' => 'info'],
                            'script'              => ['label' => 'Run Script',     'icon' => 'bi-file-code',         'btnColor' => 'info'],
                            'file_write'          => ['label' => 'Write File',     'icon' => 'bi-file-earmark-text', 'btnColor' => 'info'],
                            'wait'                => ['label' => 'Wait / Pause',   'icon' => 'bi-hourglass-split',   'btnColor' => 'secondary'],
                            'harvest'             => ['label' => 'Gather Results', 'icon' => 'bi-collection',        'btnColor' => 'success'],
                            'schedule_task'       => ['label' => 'Schedule',       'icon' => 'bi-clock',             'btnColor' => 'secondary'],
                            'condition'           => ['label' => 'If / Then',      'icon' => 'bi-signpost-split',    'btnColor' => 'secondary'],
                            'switch'              => ['label' => 'Switch',         'icon' => 'bi-signpost-2',        'btnColor' => 'secondary'],
                            'shopify_theme'       => ['label' => 'Shopify Theme',  'icon' => 'bi-palette',           'btnColor' => 'success'],
                            'shopify_liquid_eval' => ['label' => 'Shopify Liquid', 'icon' => 'bi-code-slash',        'btnColor' => 'success'],
                            'stitch_mcp'          => ['label' => 'Stitch MCP',     'icon' => 'bi-plug',              'btnColor' => 'info'],
                            'llm_call'            => ['label' => 'LLM Call',       'icon' => 'bi-chat-dots',         'btnColor' => 'primary'],
                            'datastore_query'     => ['label' => 'Datastore',      'icon' => 'bi-database',          'btnColor' => 'info'],
                            'reaper'              => ['label' => 'Cleanup',        'icon' => 'bi-trash',             'btnColor' => 'danger'],
                        ];
                        ?>
                        <div class="row g-2" id="stepTypeButtonGrid">
                            <?php foreach ($primaryTypes as $type => $display): ?>
                            <div class="col-4">
                                <button type="button" class="btn btn-outline-<?= $display['btnColor'] ?> w-100 py-3 step-type-btn"
                                        data-type="<?= $type ?>" onclick="selectStepType('<?= $type ?>')">
                                    <i class="bi <?= $display['icon'] ?> fs-3 d-block mb-1"></i>
                                    <span class="fw-bold"><?= $display['label'] ?></span>
                                </button>
                            </div>
                            <?php endforeach; ?>
                            <div class="col-12">
                                <button type="button" class="btn btn-link w-100 text-muted"
                                        data-bs-toggle="collapse" data-bs-target="#moreStepTypes">
                                    <i class="bi bi-three-dots"></i> More step types...
                                </button>
                            </div>
                            <div class="collapse col-12" id="moreStepTypes">
                                <div class="row g-2">
                                    <?php foreach ($moreTypes as $type => $display): ?>
                                    <div class="col-4">
                                        <button type="button" class="btn btn-outline-<?= $display['btnColor'] ?> w-100 py-3 step-type-btn"
                                                data-type="<?= $type ?>" onclick="selectStepType('<?= $type ?>')">
                                            <i class="bi <?= $display['icon'] ?> fs-3 d-block mb-1"></i>
                                            <span class="fw-bold small"><?= $display['label'] ?></span>
                                        </button>
                                    </div>
                                    <?php endforeach; ?>
                                </div>
                        ...
                        <!-- Hidden radio buttons for form submission compatibility -->
                        <div style="display:none;">
                            <?php foreach ($stepTypes as $type => $info): ?>
                            <input type="radio" name="step_type" id="type_<?= $type ?>" value="<?= $type ?>">
                            <?php endforeach; ?>
                        </div>
                    </div>
                    ...
                    <div class="card border-secondary mt-3" id="advancedOptionsCard">
                        <div class="card-header bg-light py-2 d-flex justify-content-between align-items-center"
                            ...
                            <span>
                                ...
                                <span class="badge bg-secondary ms-1" id="advancedOptionsSummary"></span>
                            </span>
                            ...
                            <div class="card-body">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select" name="input_source" id="inputSource">
                                        <option value="context">Pipeline context (first step default)</option>
                                        <option value="stdin">Previous step's output</option>
                                        <option value="getfrom">Get from specific step</option>
                                    </select>
                                    <small class="text-muted" id="inputSourceHint">Auto-set based on step position</small>
                                </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                ?>
                <div class="mb-3">
                    <label class="form-label" for="trigger_<?= h($key) ?>">
                    ...
                    <textarea class="form-control trigger-field" id="trigger_<?= h($key) ?>"
                        data-field="<?= h($key) ?>"
                        placeholder="<?= h($description) ?>"
                        rows="3"
                        <?= $isRequired ? 'required' : '' ?>></textarea>
                    <?php else: ?>
                    <input type="text" class="form-control trigger-field" id="trigger_<?= h($key) ?>"
                        data-field="<?= h($key) ?>"
                        placeholder="<?= h($description) ?>"
                        <?= $isRequired ? 'required' : '' ?>>
                    <?php endif; ?>
...
<!-- flatpickr (date/time picker for schedule_task) -->
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/flatpickr/dist/flatpickr.min.css">
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/flatpickr/dist/themes/dark.css">
<script src="https://cdn.jsdelivr.net/npm/flatpickr"></script>

<!-- FullCalendar 6.x (calendar widget for schedule_task) -->
...
.step-type-btn[data-type="direct_exec"].active { background-color: var(--bs-dark); border-color: var(--bs-dark); }
.step-type-btn[data-type="parser"].active { background-color: var(--bs-secondary); border-color: var(--bs-secondary); }
.step-type-btn[data-type="ai_agent"].active { background-color: var(--bs-primary); border-color: var(--bs-primary); }
.step-type-btn[data-type="shopify_graphql"].active { background-color: var(--bs-success); border-color: var(--bs-success); }
.step-type-btn[data-type="webhook_out"].active { background-color: var(--bs-warning); border-color: var(--bs-warning); color: var(--bs-dark) !important; }
.step-type-btn[data-type="mailgun"].active { background-color: var(--bs-danger); border-color: var(--bs-danger); }
.step-type-btn[data-type="mcp_call"].active { background-color: var(--bs-info); border-color: var(--bs-info); }
.step-type-btn[data-type="script"].active { background-color: var(--bs-info); border-color: var(--bs-info); }
.step-type-btn[data-type="file_write"].active { background-color: var(--bs-info); border-color: var(--bs-info); }
.step-type-btn[data-type="wait"].active { background-color: var(--bs-secondary); border-color: var(--bs-secondary); }
.step-type-btn[data-type="harvest"].active { background-color: var(--bs-success); border-color: var(--bs-success); }
.step-type-btn[data-type="schedule_task"].active { background-color: var(--bs-secondary); border-color: var(--bs-secondary); }
.step-type-btn[data-type="condition"].active { background-color: var(--bs-secondary); border-color: var(--bs-secondary); }
.step-type-btn[data-type="switch"].active { background-color: var(--bs-secondary); border-color: var(--bs-secondary); }
.step-type-btn[data-type="shopify_theme"].active { background-color: var(--bs-success); border-color: var(--bs-success); }
.step-type-btn[data-type="shopify_liquid_eval"].active { background-color: var(--bs-success); border-color: var(--bs-success); }
.step-type-btn[data-type="stitch_mcp"].active { background-color: var(--bs-info); border-color: var(--bs-info); }
.step-type-btn[data-type="llm_call"].active { background-color: var(--bs-primary); border-color: var(--bs-primary); }
.step-type-btn[data-type="datastore_query"].active { background-color: var(--bs-info); border-color: var(--bs-info); }
.step-type-btn[data-type="reaper"].active { background-color: var(--bs-danger); border-color: var(--bs-danger); }
/* Advanced Options toggle chevron rotation */
#advancedOptionsChevron {
    transition: transform 0.2s;
}
#advancedOptionsChevron.rotated {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const advancedEl = document.getElementById('advancedOptions');
    if (advancedEl) {
        advancedEl.addEventListener('show.bs.collapse', function() {
            document.getElementById('advancedOptionsChevron').classList.add('rotated');
        });
        advancedEl.addEventListener('hide.bs.collapse', function() {
            document.getElementById('advancedOptionsChevron').classList.remove('rotated');
        });
    }
...
function selectStepType(type) {
    ...
    if (btn) {
        ...
        // If it's in the "More" section, expand it
        if (btn.closest('#moreStepTypes')) {
            const moreSection = document.getElementById('moreStepTypes');
            if (!moreSection.classList.contains('show')) {
                new bootstrap.Collapse(moreSection, { toggle: true });
            }
        }
    }

    // Set hidden radio (still needed for buildConfig)
    const radio = document.getElementById('type_' + type);
    if (radio) {
        radio.checked = true;
    }

    // Show config panel and update display
...
function onStepTypeChange(type) {
    ...
    // Sync button grid highlight (for calls from editStep that go through onStepTypeChange directly)
    document.querySelectorAll('.step-type-btn').forEach(btn => {
        btn.classList.remove('active');
    });
    const activeBtn = document.querySelector('.step-type-btn[data-type="' + type + '"]');
    if (activeBtn) {
        activeBtn.classList.add('active');
        // If it's in the "More" section, expand it
        if (activeBtn.closest('#moreStepTypes')) {
            const moreSection = document.getElementById('moreStepTypes');
            if (!moreSection.classList.contains('show')) {
                new bootstrap.Collapse(moreSection, { toggle: true });
            }
...
function addStep(row, col) {
    ...
    // Smart input source default: first column uses context, later columns use stdin
    const prevColStep = document.querySelector(
        '.drop-zone[data-row="' + row + '"][data-col="' + (col - 1) + '"] .step-cell'
    );
    if (col === 0 || !prevColStep) {
        document.getElementById('inputSource').value = 'context';
    } else {
        document.getElementById('inputSource').value = 'stdin';
    }

    // Collapse advanced options for new steps
    bootstrap.Collapse.getOrCreateInstance(document.getElementById('advancedOptions'), { toggle: false }).hide();
    // Collapse "More step types" for new steps
    bootstrap.Collapse.getOrCreateInstance(document.getElementById('moreStepTypes'), { toggle: false }).hide();
    ...
    fetch('/pipelines/getstep/' + pipelineId + '?step_id=' + stepId)
        ...
        .then(data => {
            if (data.success) {
                ...
                const advBs = bootstrap.Collapse.getOrCreateInstance(advancedCollapse, { toggle: false });
                if (hasNonDefaults) {
                    advBs.show();
                } else {
                    advBs.hide();
...
function populateConfig(type, config) {
    ...
    switch (type) {
        ...
        case 'file_write':
            ...
            break;
        case 'schedule_task':
            document.querySelector('[name="config_schedule_task_type"]').value = config.task_type || 'execute_pipeline';
            // Timing mode
            const timingMode = config.timing_mode || 'delay';
            const timingRadio = document.querySelector(`input[name="config_schedule_timing_mode"][value="${timingMode}"]`);
            if (timingRadio) timingRadio.checked = true;
            if (typeof toggleScheduleTimingMode === 'function') toggleScheduleTimingMode(timingMode);
            // Delay fields
            document.querySelector('[name="config_schedule_delay"]').value = config.delay || 1;
            document.querySelector('[name="config_schedule_delay_unit"]').value = config.delay_unit || 3600;
            // Datetime fields
            const dtInput = document.querySelector('[name="config_schedule_datetime"]');
            if (dtInput) {
                dtInput.value = config.scheduled_datetime || '';
                if (dtInput._flatpickr && config.scheduled_datetime) {
                    dtInput._flatpickr.setDate(config.scheduled_datetime, false);
                }
            }
            const tzSelect = document.querySelector('[name="config_schedule_timezone"]');
            if (tzSelect) tzSelect.value = config.timezone || 'America/New_York';
            // Recurring fields
            const recurTypeSelect = document.querySelector('[name="config_schedule_recur_type"]');
            if (recurTypeSelect) recurTypeSelect.value = config.schedule_type || 'daily';
            if (typeof toggleRecurringConfig === 'function') toggleRecurringConfig(config.schedule_type || 'daily');
            // Recurring sub-fields
            const recurConfig = config.schedule_config || {};
            const minutelyInterval = document.querySelector('[name="config_schedule_minutely_interval"]');
            if (minutelyInterval) minutelyInterval.value = recurConfig.interval || 5;
            const hourlyInterval = document.querySelector('[name="config_schedule_hourly_interval"]');
            if (hourlyInterval) hourlyInterval.value = recurConfig.interval || 1;
            const hourlyMinute = document.querySelector('[name="config_schedule_hourly_minute"]');
            if (hourlyMinute) hourlyMinute.value = recurConfig.minute || 0;
            const dailyHour = document.querySelector('[name="config_schedule_daily_hour"]');
            if (dailyHour) dailyHour.value = recurConfig.hour || 9;
            const dailyMinute = document.querySelector('[name="config_schedule_daily_minute"]');
            if (dailyMinute) dailyMinute.value = recurConfig.minute || 0;
            const weeklyHour = document.querySelector('[name="config_schedule_weekly_hour"]');
            if (weeklyHour) weeklyHour.value = recurConfig.hour || 9;
            const weeklyMinute = document.querySelector('[name="config_schedule_weekly_minute"]');
            if (weeklyMinute) weeklyMinute.value = recurConfig.minute || 0;
            (recurConfig.days_of_week || []).forEach(d => {
                const cb = document.querySelector(`input[name="config_schedule_weekly_days"][value="${d}"]`);
                if (cb) cb.checked = true;
            });
            const monthlyDay = document.querySelector('[name="config_schedule_monthly_day"]');
            if (monthlyDay) monthlyDay.value = recurConfig.day_of_month || 1;
            const monthlyHour = document.querySelector('[name="config_schedule_monthly_hour"]');
            if (monthlyHour) monthlyHour.value = recurConfig.hour || 9;
            const monthlyMinuteEl = document.querySelector('[name="config_schedule_monthly_minute"]');
            if (monthlyMinuteEl) monthlyMinuteEl.value = recurConfig.minute || 0;
            const cronExpr = document.querySelector('[name="config_schedule_cron_expression"]');
            if (cronExpr) cronExpr.value = recurConfig.cron_expression || '0 9 * * *';
            const overlapSelect = document.querySelector('[name="config_schedule_on_overlap"]');
            if (overlapSelect) overlapSelect.value = config.on_overlap || 'skip';
            // Pipeline config
            document.querySelector('[name="config_schedule_pipeline_id"]').value = config.pipeline_id || '';
            if (config.pipeline_id) {
                loadPipelineSteps(config.pipeline_id, config.entry_step || '');
            }
            const inputDataEl = document.querySelector('[name="config_schedule_input_data"]');
            if (inputDataEl) {
                inputDataEl.value = config.input_data ? (typeof config.input_data === 'string' ? config.input_data : JSON.stringify(config.input_data, null, 2)) : '';
            }
            // Webhook config
            const whUrl = document.querySelector('[name="config_schedule_webhook_url"]');
            if (whUrl) whUrl.value = config.webhook_url || '';
            const whMethod = document.querySelector('[name="config_schedule_webhook_method"]');
            if (whMethod) whMethod.value = config.webhook_method || 'POST';
            const whHeaders = document.querySelector('[name="config_schedule_webhook_headers"]');
            if (whHeaders) whHeaders.value = config.webhook_headers ? JSON.stringify(config.webhook_headers) : '';
            const whBody = document.querySelector('[name="config_schedule_webhook_body"]');
            if (whBody) whBody.value = config.webhook_body || '';
            // Toggle task type visibility
            if (typeof toggleScheduleTaskConfig === 'function') {
                toggleScheduleTaskConfig(document.querySelector('[name="config_schedule_task_type"]'));
            }
            break;
    }
}
...
function buildConfig() {
    ...
    switch (type) {
        ...
        case 'file_write':
            ...
            break;
        case 'schedule_task': {
            const taskType = document.querySelector('[name="config_schedule_task_type"]').value;
            const timingMode = document.querySelector('input[name="config_schedule_timing_mode"]:checked')?.value || 'delay';
            config = { task_type: taskType, timing_mode: timingMode };
            // Delay fields
            config.delay = parseInt(document.querySelector('[name="config_schedule_delay"]').value) || 1;
            config.delay_unit = parseInt(document.querySelector('[name="config_schedule_delay_unit"]').value) || 3600;
            // Datetime fields
            const dtVal = document.querySelector('[name="config_schedule_datetime"]')?.value || '';
            if (dtVal) config.scheduled_datetime = dtVal;
            const tzVal = document.querySelector('[name="config_schedule_timezone"]')?.value;
            if (tzVal) config.timezone = tzVal;
            // Recurring fields
            if (timingMode === 'recurring') {
                config.schedule_type = document.querySelector('[name="config_schedule_recur_type"]')?.value || 'daily';
                config.on_overlap = document.querySelector('[name="config_schedule_on_overlap"]')?.value || 'skip';
                const rc = {};
                switch (config.schedule_type) {
                    case 'minutely':
                        rc.interval = parseInt(document.querySelector('[name="config_schedule_minutely_interval"]')?.value) || 5;
                        break;
                    case 'hourly':
                        rc.interval = parseInt(document.querySelector('[name="config_schedule_hourly_interval"]')?.value) || 1;
                        rc.minute = parseInt(document.querySelector('[name="config_schedule_hourly_minute"]')?.value) || 0;
                        break;
                    case 'daily':
                        rc.hour = parseInt(document.querySelector('[name="config_schedule_daily_hour"]')?.value) || 9;
                        rc.minute = parseInt(document.querySelector('[name="config_schedule_daily_minute"]')?.value) || 0;
                        break;
                    case 'weekly':
                        rc.hour = parseInt(document.querySelector('[name="config_schedule_weekly_hour"]')?.value) || 9;
                        rc.minute = parseInt(document.querySelector('[name="config_schedule_weekly_minute"]')?.value) || 0;
                        rc.days_of_week = [];
                        document.querySelectorAll('input[name="config_schedule_weekly_days"]:checked').forEach(cb => {
                            rc.days_of_week.push(parseInt(cb.value));
                        });
                        break;
                    case 'monthly':
                        rc.day_of_month = parseInt(document.querySelector('[name="config_schedule_monthly_day"]')?.value) || 1;
                        rc.hour = parseInt(document.querySelector('[name="config_schedule_monthly_hour"]')?.value) || 9;
                        rc.minute = parseInt(document.querySelector('[name="config_schedule_monthly_minute"]')?.value) || 0;
                        break;
                    case 'cron':
                        rc.cron_expression = document.querySelector('[name="config_schedule_cron_expression"]')?.value || '0 9 * * *';
                        break;
                }
                config.schedule_config = rc;
            }
            // Pipeline config
            if (taskType === 'execute_pipeline') {
                config.pipeline_id = document.querySelector('[name="config_schedule_pipeline_id"]').value;
                config.entry_step = document.querySelector('[name="config_schedule_entry_step"]')?.value || '';
                const inputStr = document.querySelector('[name="config_schedule_input_data"]')?.value || '';
                if (inputStr) {
                    try { config.input_data = JSON.parse(inputStr); } catch(e) { config.input_data = inputStr; }
                }
            }
            // Webhook config
            if (taskType === 'webhook_call') {
                config.webhook_url = document.querySelector('[name="config_schedule_webhook_url"]')?.value || '';
                config.webhook_method = document.querySelector('[name="config_schedule_webhook_method"]')?.value || 'POST';
                const whHdrStr = document.querySelector('[name="config_schedule_webhook_headers"]')?.value || '{}';
                try { config.webhook_headers = JSON.parse(whHdrStr); } catch(e) { config.webhook_headers = {}; }
                config.webhook_body = document.querySelector('[name="config_schedule_webhook_body"]')?.value || '';
            }
            break;
        }
    }
...
function checkEngineStatus() {
    fetch('/pipelines/enginestatus', { headers: { 'Accept': 'application/json' } })
        .then(r => r.json())
        .then(data => {
            ...
            if (!d.exists || d.status === 'none') {
                btn.className = 'btn btn-outline-secondary btn-sm';
                icon.className = 'bi bi-cpu';
                label.textContent = 'Engine Off';
                btn.title = 'Click to start the Pipeline Engine (faster execution)';
            } else if (d.status === 'running' && d.healthy) {
                btn.className = 'btn btn-success btn-sm';
                icon.className = 'bi bi-cpu-fill';
                label.textContent = 'Engine On';
                btn.title = 'Pipeline Engine running on port ' + (d.port || '?') + '. Click to stop.';
            } else if (d.status === 'starting') {
                btn.className = 'btn btn-warning btn-sm';
                icon.className = 'bi bi-hourglass-split';
                label.textContent = 'Starting...';
                btn.title = 'Engine is starting up...';
                setTimeout(checkEngineStatus, 2000);
            } else {
                btn.className = 'btn btn-danger btn-sm';
                icon.className = 'bi bi-exclamation-triangle';
                label.textContent = d.status || 'Error';
                btn.title = d.error_message || 'Engine error. Click to restart.';
            }
        })
        .catch(() => {});
}
...
function toggleEngine() {
    ...
    const endpoint = isRunning ? '/pipelines/stopengine' : '/pipelines/startengine';
    label.textContent = isRunning ? 'Stopping...' : 'Starting...';
    btn.disabled = true;
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            showNotification(data.message || 'Engine operation failed', 'danger');
            checkEngineStatus();
        }
    })
    .catch(err => {
        btn.disabled = false;
        showNotification('Engine operation failed: ' + err.message, 'danger');
        checkEngineStatus();
    });
}
