...
class PipelineExecutor {
    ...
    /** @var array Temp SSH key files to clean up */
    private array $tempSSHKeyFiles = [];

    // Performance timing (enabled via setTimingEnabled)
    private bool $timingEnabled = false;
    private array $timings = [];

    // Optional workspace override (for engine context where Flight is unavailable)
    private ?string $workspace = null;

    public function __construct(int $runId, $logger = null, ?string $workspace = null) {
        $this->runId = $runId;
        $this->logger = $logger;
    ...
    /**
     * Cleanup temp SSH key files on destruction
     */
    public function __destruct() {
        foreach ($this->tempSSHKeyFiles as $keyFile) {
            SSHKeyService::cleanupTempKeyFile($keyFile);
        }
        $this->tempSSHKeyFiles = [];
    }
                            ...
                            for ($resetIdx = $targetIdx; $resetIdx <= $i; $resetIdx++) {
                                if (isset($stepsArray[$resetIdx])) {
                                    ...
                                    if ($loopStepRun && in_array($loopStepRun->status, ['success', 'awaiting_input', 'skipped'])) {
                                        ...
                                        // Archive step output to _prev before clearing
                                        // This allows next loop iteration to access previous results
                                        if (isset($this->context[$loopStep->step_name])) {
                                            $this->context[$loopStep->step_name . '_prev'] = $this->context[$loopStep->step_name];
                                        }
    ...
    private function getBaseUrl(): string {
        ...
        if ($workspace && $workspace !== 'default') {
            return SiteConfig::getWorkspaceUrl($workspace);
        }
    ...
    public function execute(): bool {
        try {
            ...
            // Initialize context from pipeline defaults, trigger data, AND input data
            ...
            $inputData = json_decode($this->run->context_json ?: '{}', true);
            $this->context = array_merge($defaultContext, $triggerData, $inputData);
            ...
            $workspace = $this->workspace
                ?? (class_exists('\Flight', false) ? \Flight::get('workspace') : null)
                ?? $_SERVER['WORKSPACE']
                ?? null;
            if (!$workspace) {
            ...
            $baseUrl = (class_exists('\Flight', false) ? \Flight::get('app.baseurl') : null)
                ?? "https://{$workspace}.myctobot.ai";
    ...
    private function executeStep($step, array $input): array {
        ...
        switch ($step->step_type) {
            ...
            case 'email_out': // alias for mailgun
            ...
            case 'shopify_theme':
                return $this->executeShopifyTheme($config, $input);

            case 'shopify_liquid_eval':
                return $this->executeShopifyLiquidEval($config, $input);
            ...
            case 'datastore_query':
                return $this->executeDatastoreQuery($config, $input);

            case 'llm_call':
                return $this->executeLlmCall($config, $input, $timeout);

            case 'shopify_crawler':
                return $this->executeShopifyCrawler($config, $input, $timeout);

            case 'stitch_mcp':
                return $this->executeStitchMcp($config, $input);

            case 'linkedin_enrich':
                return $this->executeLinkedInEnrich($config, $input, $timeout);
    ...
    private function executeDirectExec(array $config, array $input, int $timeout): array {
        ...
        if (!empty($workstationId)) {
            ...
            if ($workstation->id && $workstation->ssh_user && $workstation->host) {
                ...
                // If workstation has SSH key configured, prepare temp key file
                if (!empty($workstation->sshkey_id)) {
                    $tempKeyFile = SSHKeyService::prepareTempKeyFile((int)$workstation->sshkey_id);
                    if ($tempKeyFile) {
                        $this->tempSSHKeyFiles[] = $tempKeyFile;
                        $sshKeyArg = "-i " . escapeshellarg($tempKeyFile) . " ";
                        $this->log('debug', "Using SSH key ID {$workstation->sshkey_id} for workstation {$workstationId}");
                    }
                }

                // Build SSH command with key, port, and strict host options
    ...
    private function executeAIAgent(array $config, array $input, int $timeout): array {
        ...
        $stepSystemPrompt = $config['system_prompt'] ?? null;
        ...
        // System prompt priority: step config > agent's system_prompt > default
...
PIPELINE;
        ...
        try {
            ...
            if ($useOllama) {
                $result = $this->executeOllamaAgent($agent, $provider, $providerConfig, $userMessage, $systemPrompt, $timeout, $maxTokens);
     ...
     */
    private function executeOllamaAgent($agent, string $provider, array $providerConfig, string $userMessage, string $systemPrompt, int $timeout, int $maxTokens = 4096): array {
        // Determine host and model based on provider type
        if ($provider === 'ollama') {
            $ollamaHost = $providerConfig['host'] ?? $providerConfig['base_url'] ?? 'http://127.0.0.1:11435';
            $ollamaModel = $providerConfig['model'] ?? '';
        } else {
            // claude_cli with use_ollama
            $ollamaHost = $providerConfig['ollama_host'] ?? 'http://127.0.0.1:11435';
     ...
     *   connection_id - Specific Mailgun connection ID (optional, supports variable substitution)
    ...
    private function executeEmailOut(array $config, array $input): array {
        ...
        // Connection ID for specific Mailgun account (optional)
        $connectionId = !empty($config['connection_id'])
            ? (int) $this->substituteVariables($config['connection_id'])
            : null;

        try {
            $mailgun = new MailgunService($connectionId);
    ...
    private function executeScheduleTask(array $config, array $input): array {
        ...
        $timingMode = $config['timing_mode'] ?? 'delay';

        // Handle recurring mode - creates a scheduledrecurring record instead
        if ($timingMode === 'recurring') {
            return $this->executeScheduleTaskRecurring($config, $input);
        }

        // Calculate scheduledAt based on timing mode
        $delaySeconds = 0;
        $scheduledAt = null;

        if ($timingMode === 'datetime') {
            // Datetime mode: parse scheduled_datetime with timezone
            $datetimeStr = $config['scheduled_datetime'] ?? '';
            $timezone = $config['timezone'] ?? 'UTC';

            if (empty($datetimeStr)) {
                return ['success' => false, 'error' => 'Datetime mode requires scheduled_datetime'];
            }

            try {
                $tz = new \DateTimeZone($timezone);
                $scheduledDt = new \DateTime($datetimeStr, $tz);
                $now = new \DateTime('now', $tz);

                if ($scheduledDt <= $now) {
                    return ['success' => false, 'error' => 'Scheduled datetime is in the past'];
                }

                $scheduledAt = $scheduledDt->format('Y-m-d H:i:s');
                $delaySeconds = $scheduledDt->getTimestamp() - $now->getTimestamp();
            } catch (\Exception $e) {
                return ['success' => false, 'error' => 'Invalid datetime: ' . $e->getMessage()];
            }
        } else {
            // Delay mode (default)
            ...
            if ($delaySeconds === 0 && isset($config['delay'])) {
                ...
                $delayUnit = (int) ($config['delay_unit'] ?? 3600);
        ...
        try {
            ...
            $this->log('info', "Scheduled task created: type={$taskType}, timing={$timingMode}, scheduled_at={$scheduledAt}, task_id={$task->id}");
    ...
    /**
     * Handle recurring timing mode for schedule_task step.
     * Creates a scheduledrecurring record via SchedulerService.
     */
    private function executeScheduleTaskRecurring(array $config, array $input): array {
        $taskType = $config['task_type'] ?? 'execute_pipeline';

        if ($taskType !== 'execute_pipeline') {
            return ['success' => false, 'error' => 'Recurring mode only supports execute_pipeline task type'];
        }

        $scheduleType = $config['schedule_type'] ?? 'daily';
        $scheduleConfig = $config['schedule_config'] ?? [];
        $timezone = $config['timezone'] ?? 'UTC';
        $onOverlap = $config['on_overlap'] ?? 'skip';

        $pipelineId = $config['pipeline_id'] ?? null;
        if ($pipelineId === '_self') {
            $pipelineId = $this->pipeline->id ?? null;
        }

        if (!$pipelineId) {
            return ['success' => false, 'error' => 'Recurring schedule requires a pipeline_id'];
        }
        ...
        try {
            ...
            $scheduleId = SchedulerService::create([
                ...
                'name' => $scheduleName,
                'description' => 'Created by schedule_task step in run #' . $this->runId,
                'schedule_type' => $scheduleType,
                'schedule_config' => $scheduleConfig,
                'timezone' => $timezone,
                'on_overlap' => $onOverlap,
                'entry_step' => $config['entry_step'] ?? null,
                'input_data' => $inputData,
            ]);

            $this->log('info', "Recurring schedule created: type={$scheduleType}, schedule_id={$scheduleId}");
            ...
            $this->log('error', "Failed to create recurring schedule: " . $e->getMessage());
            return ['success' => false, 'error' => $e->getMessage()];
        }
    }
    ...
    private function substituteVariables(string $template): string {
        // Replace time variables {time.xxx}
        $template = preg_replace_callback('/\{time\.([^}]+)\}/', function($matches) {
            $key = $matches[1];
            switch ($key) {
                case 'now': return date('Y-m-d H:i:s');
                case 'epoch': return (string) time();
                case 'date': return date('Y-m-d');
                case 'time': return date('H:i:s');
                case 'microtime': return sprintf('%.6f', microtime(true));
                case 'iso8601': return date('c');
                case 'timestamp_ms': return (string) round(microtime(true) * 1000);
                default: return '';
            }
        }, $template);
    ...
    private function completeRun(string $status, ?string $errorMessage = null): void {
        ...
        // Set progress to 100% when completed (regardless of steps skipped due to branching)
        if ($status === 'completed') {
            $this->run->progress_percent = 100;
        }
        ...
        // Decrement scheduler's current_running counter if this was a scheduled run
        $this->notifyScheduleCompletion();
    ...
    /**
     * Notify the scheduler that this run has finished (decrements current_running).
     * Called from all run completion paths — both completeRun() and direct status assignments.
     */
    private function notifyScheduleCompletion(): void {
        $scheduleId = (int) ($this->context['_schedule_id'] ?? 0);
        if ($scheduleId <= 0) {
            return;
        }
        ...
        try {
            require_once __DIR__ . '/SchedulerService.php';
            SchedulerService::markCompleted($scheduleId);
            $this->log('info', "Decremented current_running for schedule #{$scheduleId}");
        } catch (\Throwable $e) {
            $this->log('error', "Failed to notify schedule #{$scheduleId} of completion: " . $e->getMessage());
        }
    }
     ...
     *
     * Config options:
     *   connection_id  - Shopify connection ID from unified connections table (required)
    ...
    private function executeShopifyGraphql(array $config, array $input, int $timeout): array {
        ...
        if (is_string($connectionId)) {
            ...
            if (is_numeric($connectionId)) {
                ...
                // Try to resolve by store domain or connection name
                $conn = \app\Bean::findOne('connections',
                    'connector_type = ? AND (external_eid LIKE ? OR connection_name LIKE ? OR external_name LIKE ?)',
                    ['shopify', "%{$connectionId}%", "%{$connectionId}%", "%{$connectionId}%"]
                );
                $connectionId = $conn ? (int)$conn->id : null;
            }
        }

        if (empty($connectionId)) {
            // Fall back to the first active connection if none specified
            $defaultConn = \app\Bean::findOne('connections', 'connector_type = ? AND enabled = 1 ORDER BY id ASC', ['shopify']);
            if ($defaultConn) {
                $connectionId = (int)$defaultConn->id;
                $shopLabel = $defaultConn->external_eid ?? $defaultConn->external_name ?? 'unknown';
                $this->log('info', "Using default Shopify connection: {$connectionId} ({$shopLabel})");
            } else {
                return ['success' => false, 'error' => 'Shopify connection_id is required and no default connection found'];
            }
        }
        ...
        foreach ($variables as $key => $value) {
            if (is_string($value)) {
                ...
                // Skip empty-string variables — GraphQL treats omitted nullable
                // variables as null (use default). This prevents sending invalid
                // enum values like "" for ProductSortKeys.
                if ($substituted === '') {
                    continue;
                }
    ...
    /**
     * Execute Shopify theme operations
     *
     * Manages theme assets, script tags, banners, and announcements.
     *
     * Config options:
     *   connection_id - Shopify connection ID (required)
     *   operation     - Operation to perform:
     *                   Script Tags: install_widget, remove_widget, list_scripts
     *                   Theme Assets: upload_banner, get_asset, update_asset, delete_asset
     *                   Announcements: inject_announcement
     *                   Themes: list_themes, get_main_theme
     *
     * Operation-specific options:
     *   install_widget:
     *     script_url   - URL of script to inject
     *     script_event - Load event: 'onload' or 'DOMContentLoaded'
     *
     *   remove_widget:
     *     script_url   - URL pattern to match for removal
     *
     *   upload_banner:
     *     banner_name  - Name for the banner asset
     *     theme_id     - Target theme ID (optional, defaults to main)
     *     banner_source - Source type: 'url', 'base64', or 'path'
     *     banner_url   - URL to download image from (if source=url)
     *     banner_base64 - Base64 encoded image data (if source=base64)
     *     banner_path  - Local file path (if source=path)
     *
     *   get_asset, update_asset, delete_asset:
     *     asset_key    - Asset path (e.g., 'assets/custom.css')
     *     asset_theme_id - Theme ID (optional)
     *     asset_content - New content (for update_asset only)
     *
     *   inject_announcement:
     *     announcement_message - Text to display
     *     announcement_bg      - Background color (hex)
     *     announcement_text    - Text color (hex)
     *     announcement_link    - Optional link URL
     *     announcement_expires - Optional ISO datetime
     */
    private function executeShopifyTheme(array $config, array $input): array {
        require_once __DIR__ . '/ShopifyThemeService.php';

        $connectionId = $config['connection_id'] ?? null;
        $operation = $config['operation'] ?? '';

        // Substitute variables in connection_id
        if (is_string($connectionId)) {
            $connectionId = $this->substituteVariables($connectionId);
        ...
        if (empty($operation)) {
            return ['success' => false, 'error' => 'Theme operation is required'];
        }

        $this->log('info', "Executing Shopify Theme operation", [
            'connection_id' => $connectionId,
            'operation' => $operation
        ]);

        try {
            $service = new ShopifyThemeService($connectionId);

            switch ($operation) {
                // ================= Script Tags =================
                case 'install_widget':
                    $scriptUrl = $this->substituteVariables($config['script_url'] ?? '');
                    $scriptEvent = $config['script_event'] ?? 'onload';

                    if (empty($scriptUrl)) {
                        return ['success' => false, 'error' => 'script_url is required for install_widget'];
                    }

                    $result = $service->installWidget($scriptUrl);
                    return [
                        'success' => true,
                        'output' => ['script_tag' => $result, 'action' => 'installed'],
                        'stdout' => "Installed widget script: {$scriptUrl}"
                    ];

                case 'remove_widget':
                    $urlPattern = $this->substituteVariables($config['script_url'] ?? '');

                    if (empty($urlPattern)) {
                        return ['success' => false, 'error' => 'script_url pattern is required for remove_widget'];
                    }

                    $removed = $service->removeWidget($urlPattern);
                    return [
                        'success' => true,
                        'output' => ['removed' => $removed, 'pattern' => $urlPattern],
                        'stdout' => $removed ? "Removed widget matching: {$urlPattern}" : "No widget found matching: {$urlPattern}"
                    ];

                case 'list_scripts':
                    $scripts = $service->listScriptTags();
                    return [
                        'success' => true,
                        'output' => ['script_tags' => $scripts, 'count' => count($scripts)],
                        'stdout' => json_encode($scripts, JSON_PRETTY_PRINT)
                    ];

                // ================= Theme Assets =================
                case 'upload_banner':
                    $bannerName = $this->substituteVariables($config['banner_name'] ?? 'banner');
                    $themeId = $config['theme_id'] ?? null;
                    if ($themeId) {
                        $themeId = (int)$this->substituteVariables((string)$themeId);
                    }

                    $source = $config['banner_source'] ?? 'url';
                    $tempFile = null;

                    // Prepare file based on source
                    if ($source === 'url') {
                        $imageUrl = $this->substituteVariables($config['banner_url'] ?? '');
                        if (empty($imageUrl)) {
                            return ['success' => false, 'error' => 'banner_url is required for source=url'];
                        }
                        ...
                        $tempFile = sys_get_temp_dir() . '/shopify-banner-' . uniqid() . '.' . $ext;
                        file_put_contents($tempFile, $imageData);
                        $filePath = $tempFile;

                    } elseif ($source === 'base64') {
                        $base64Data = $this->substituteVariables($config['banner_base64'] ?? '');
                        if (empty($base64Data)) {
                            return ['success' => false, 'error' => 'banner_base64 is required for source=base64'];
                        }
                        ...
                        $filePath = $this->substituteVariables($config['banner_path'] ?? '');
                        if (empty($filePath) || !file_exists($filePath)) {
                            return ['success' => false, 'error' => "banner_path is required and must exist: {$filePath}"];
                        }
                    }

                    try {
                        $result = $service->uploadBanner($filePath, $bannerName, $themeId);
                        return [
                            'success' => true,
                            'output' => ['asset' => $result, 'public_url' => $result['public_url'] ?? null],
                            'stdout' => "Uploaded banner: " . ($result['public_url'] ?? $result['key'])
                        ];
                    } finally {
                        // Clean up temp file
                ...
                case 'get_asset':
                    $assetKey = $this->substituteVariables($config['asset_key'] ?? '');
                    $themeId = $config['asset_theme_id'] ?? null;

                    if (empty($assetKey)) {
                        return ['success' => false, 'error' => 'asset_key is required for get_asset'];
                    }

                    if (!$themeId) {
                        $mainTheme = $service->getMainTheme();
                        $themeId = $mainTheme['id'] ?? null;
                    } else {
                        $themeId = (int)$this->substituteVariables((string)$themeId);
                    }

                    $asset = $service->getAsset($themeId, $assetKey);
                    return [
                        'success' => true,
                        'output' => ['asset' => $asset],
                        'stdout' => $asset['value'] ?? ''
                ...
                case 'update_asset':
                    $assetKey = $this->substituteVariables($config['asset_key'] ?? '');
                    $assetContent = $this->substituteVariables($config['asset_content'] ?? '');
                    $themeId = $config['asset_theme_id'] ?? null;

                    if (empty($assetKey)) {
                        return ['success' => false, 'error' => 'asset_key is required for update_asset'];
                    }
                    ...
                    $asset = $service->updateAsset($themeId, $assetKey, $assetContent);
                    return [
                        'success' => true,
                        'output' => ['asset' => $asset, 'action' => 'updated'],
                        'stdout' => "Updated asset: {$assetKey}"
                    ];
                ...
                case 'delete_asset':
                    ...
                    if (empty($assetKey)) {
                        return ['success' => false, 'error' => 'asset_key is required for delete_asset'];
                    }
                    ...
                    $deleted = $service->deleteAsset($themeId, $assetKey);
                    return [
                        'success' => true,
                        'output' => ['deleted' => $deleted, 'key' => $assetKey],
                        'stdout' => $deleted ? "Deleted asset: {$assetKey}" : "Failed to delete: {$assetKey}"
                    ];

                // ================= Announcements =================
                case 'inject_announcement':
                    $announcementConfig = [
                        'message' => $this->substituteVariables($config['announcement_message'] ?? 'Sale!'),
                        'background_color' => $config['announcement_bg'] ?? '#ff0000',
                        'text_color' => $config['announcement_text'] ?? '#ffffff',
                        'link_url' => $this->substituteVariables($config['announcement_link'] ?? ''),
                        'expires_at' => $config['announcement_expires'] ?? ''
                    ];

                    $result = $service->injectAnnouncementBar($announcementConfig);
                    return [
                        'success' => true,
                        'output' => ['script_tag' => $result, 'config' => $announcementConfig],
                        'stdout' => "Injected announcement bar: " . $announcementConfig['message']
                    ];
                ...
                case 'list_themes':
                    ...
                    return [
                        'success' => true,
                        'output' => ['themes' => $themes, 'count' => count($themes)],
                        'stdout' => json_encode($themes, JSON_PRETTY_PRINT)
                    ];

                case 'get_main_theme':
                    $theme = $service->getMainTheme();
                    return [
                        'success' => true,
                        'output' => ['theme' => $theme],
                        'stdout' => $theme ? "Main theme: {$theme['name']} (ID: {$theme['id']})" : "No main theme found"
                    ];

                default:
                    return [
                        'success' => false,
                        'error' => "Unknown Shopify theme operation: {$operation}"
                    ];
            }

        } catch (\Exception $e) {
            $this->log('error', "Shopify Theme operation failed: " . $e->getMessage());
            return [
                'success' => false,
                'error' => "Shopify Theme exception: " . $e->getMessage()
            ];
        }
    }

    /**
     * Execute Shopify store crawler for RAG indexing
     *
     * Crawls a Shopify store's public pages (products, collections, pages, blog posts)
     * and indexes them in the knowledge store for the MyCTOBot Assistant.
     *
     * Config options:
     *   connection_id  - Shopify connection ID (required)
     *   mode           - Operation mode: discover, crawl, status, search, reset
     *   max_pages      - Max pages to crawl per batch (default: 10)
     *   page_types     - Array of page types to crawl: product, collection, page, article, blog
     *   search_query   - Search query (for mode=search)
     *
     * Example configs:
     ...
     *     "connection_id": 1,
     *     "mode": "search",
     *     "search_query": "lawn mower battery"
     *   }
     */
    private function executeShopifyCrawler(array $config, array $input, int $timeout): array {
        require_once __DIR__ . '/ShopifyCrawlerService.php';

        $connectionId = $config['connection_id'] ?? null;
        $mode = $config['mode'] ?? 'status';
        $maxPages = $config['max_pages'] ?? 10;
        $pageTypes = $config['page_types'] ?? [];
        ...
        // Substitute search query
        if (!empty($searchQuery)) {
            $searchQuery = $this->substituteVariables($searchQuery);
        }

        // Get workspace from context
        $workspace = $this->context['_workspace'] ?? $_SERVER['WORKSPACE'] ?? 'default';

        $this->log('info', "Executing Shopify crawler", [
            'connection_id' => $connectionId,
            'mode' => $mode,
            'workspace' => $workspace
        ]);
        ...
        try {
            $crawler = ShopifyCrawlerService::forConnection($connectionId, $workspace);
            ...
            switch ($mode) {
                ...
                case 'search':
                    if (empty($searchQuery)) {
                        return ['success' => false, 'error' => 'search_query is required for search mode'];
                    }
                    $results = $crawler->search($searchQuery);
                    $result = [
                        'success' => true,
                        'results' => $results,
                        'query' => $searchQuery,
            ...
            $this->log('info', "Shopify crawler completed", [
                'mode' => $mode,
            ...
            return [
                'success' => $result['success'] ?? true,
                'output' => $result,
                'stdout' => json_encode($result, JSON_PRETTY_PRINT)
            ];

        } catch (\Exception $e) {
            $this->log('error', "Shopify crawler failed: " . $e->getMessage());
            return [
                'success' => false,
                'error' => "Shopify crawler exception: " . $e->getMessage()
            ];
        }
    }

    /**
     * Execute a Google Stitch MCP tool call
     */
    private function executeStitchMcp(array $config, array $input): array {
        require_once __DIR__ . '/StitchClient.php';
        ...
        if (empty($connectionId)) {
            // Fall back to first active Stitch connection
            $defaultConn = \app\Bean::findOne('connections', 'connector_type = ? AND enabled = 1 ORDER BY id ASC', ['stitch']);
            if ($defaultConn) {
                $connectionId = (int)$defaultConn->id;
                $this->log('info', "Using default Stitch connection: {$connectionId}");
            } else {
                return ['success' => false, 'error' => 'Stitch connection_id is required and no default connection found'];
            }
        }

        if (empty($tool)) {
            return ['success' => false, 'error' => 'Stitch MCP tool name is required'];
        }

        // Substitute variables in tool name
        $tool = $this->substituteVariables($tool);
        ...
        // Strip "projects/" prefix from projectId — Stitch API returns full resource
        // names like "projects/12345" but tools expect just the numeric ID
        if (isset($processedArgs['projectId']) && is_string($processedArgs['projectId'])) {
            $processedArgs['projectId'] = preg_replace('#^projects/#', '', $processedArgs['projectId']);
        }

        $this->log('info', "Executing Stitch MCP tool", [
            'connection_id' => $connectionId,
            'tool' => $tool,
        ]);
        ...
        try {
            ...
            $result = $client->callMcpTool($tool, $processedArgs);

            $this->log('info', "Stitch MCP tool completed", ['tool' => $tool]);
            ...
            $this->log('error', "Stitch MCP failed: " . $e->getMessage());
            return [
                'success' => false,
                'error' => "Stitch MCP exception: " . $e->getMessage()
            ];
        }
    }

    /**
     * Execute a datastore query against an app's SQLite database
     *
     * Config options:
     *   app_slug     - The app slug (or {context.app_slug} for dynamic)
     *   operation    - Operation type: insert, update, select, delete, raw
     *   table        - Table name (required for insert/update/select/delete)
     *   data         - Data to insert/update (column => value pairs)
     *   where        - WHERE conditions (column => value pairs)
     *   options      - Query options for select: order_by, limit, offset, columns
     *   sql          - Raw SQL for 'raw' operation
     *   params       - Parameters for raw SQL
     *
     * Example configs:
     *   Insert:
     *   {
     ...
     *     "operation": "update",
     *     "table": "submissions",
     *     "data": { "classification": "{classify_ai.output.classification}" },
     *     "where": { "id": "{store_submission.output.last_insert_id}" }
     *   }
     *
     *   Select:
     *   {
     ...
     *     "table": "submissions",
     *     "where": { "status": "new" },
     *     "options": { "order_by": "created_at DESC", "limit": 10 }
     *   }
     */
    private function executeDatastoreQuery(array $config, array $input): array {
        require_once __DIR__ . '/AppDatastore.php';

        // Get workspace from context or run
        $workspace = $this->context['workspace'] ?? $_SERVER['WORKSPACE'] ?? null;
        if (empty($workspace)) {
            return [
                'success' => false,
                'error' => 'Datastore query requires workspace context'
            ];
        }

        // Get app_slug from config or fall back to context
        $appSlug = $config['app_slug'] ?? $this->context['app_slug'] ?? null;
        if (is_string($appSlug)) {
            $appSlug = $this->substituteVariables($appSlug, $input);
        }

        if (empty($appSlug)) {
            return [
                'success' => false,
                'error' => 'Datastore query requires app_slug in config or context'
            ];
        }
        ...
        // Validate table is provided for non-raw operations
        if ($operation !== 'raw' && empty($table)) {
            return [
                'success' => false,
                'error' => "Datastore {$operation} requires 'table' parameter"
            ];
        }

        $this->log('info', "Executing datastore query", [
            'workspace' => $workspace,
            'app_slug' => $appSlug,
            'operation' => $operation,
            'table' => $table
        ...
        try {
            $datastore = AppDatastore::forApp($workspace, $appSlug);
            ...
            switch ($operation) {
                case 'insert':
                    ...
                    $processedData = $this->processDatastoreData($data, $input);
                    $lastId = $datastore->insert($table, $processedData);
                    return [
                        'success' => true,
                        'output' => ['last_insert_id' => $lastId],
                        'stdout' => json_encode(['last_insert_id' => $lastId])
                    ];
                ...
                case 'update':
                    ...
                    $processedData = $this->processDatastoreData($data, $input);
                    $processedWhere = $this->processDatastoreData($where, $input);
                    $affectedRows = $datastore->update($table, $processedData, $processedWhere);
                    return [
                        'success' => true,
                        'output' => ['affected_rows' => $affectedRows],
                        'stdout' => json_encode(['affected_rows' => $affectedRows])
                    ];
                ...
                case 'select':
                    ...
                    $processedWhere = $this->processDatastoreData($where, $input);
                    $rows = $datastore->select($table, $processedWhere, $options);
                    return [
                        'success' => true,
                        'output' => ['rows' => $rows, 'count' => count($rows)],
                        'stdout' => json_encode(['rows' => $rows, 'count' => count($rows)], JSON_PRETTY_PRINT)
                    ];

                case 'delete':
                    $where = $config['where'] ?? [];
                    if (empty($where)) {
                        return ['success' => false, 'error' => 'Delete requires where clause'];
                    }
                    $processedWhere = $this->processDatastoreData($where, $input);
                    $deletedRows = $datastore->delete($table, $processedWhere);
                    return [
                        'success' => true,
                        'output' => ['deleted_rows' => $deletedRows],
                        'stdout' => json_encode(['deleted_rows' => $deletedRows])
                    ];
                ...
                case 'raw':
                    ...
                    if (empty($sql)) {
                        return ['success' => false, 'error' => 'Raw operation requires sql'];
                    }
                    // Substitute variables in sql and params
                    $sql = $this->substituteVariables($sql, $input);
                    $processedParams = [];
                    foreach ($params as $param) {
                        $processedParams[] = is_string($param) ? $this->substituteVariables($param, $input) : $param;
                    }
                    $result = $datastore->raw($sql, $processedParams);
                    // Result is array for SELECT, int for others
                    if (is_array($result)) {
                        return [
                            'success' => true,
                            'output' => ['rows' => $result, 'count' => count($result)],
                            'stdout' => json_encode(['rows' => $result], JSON_PRETTY_PRINT)
                        ];
                    }
                    return [
                        'success' => true,
                        'output' => ['affected_rows' => $result],
                        'stdout' => json_encode(['affected_rows' => $result])
                    ];

                case 'count':
                    $where = $config['where'] ?? [];
                    $processedWhere = $this->processDatastoreData($where, $input);
                    $count = $datastore->count($table, $processedWhere);
                    return [
                        'success' => true,
                        'output' => ['count' => $count],
                        'stdout' => json_encode(['count' => $count])
                    ];

                default:
                    return [
                        'success' => false,
                        'error' => "Unknown datastore operation: {$operation}"
                    ];
            }

        } catch (\Exception $e) {
            $this->log('error', "Datastore query failed: " . $e->getMessage());
            return [
                'success' => false,
                'error' => "Datastore exception: " . $e->getMessage()
            ];
        }
    }

    /**
     * Process datastore data by substituting variables
     *
     * @param array $data Key-value pairs
     * @param array $input Additional input context
     * @return array Processed data
     */
    private function processDatastoreData(array $data, array $input): array {
        $processed = [];
        foreach ($data as $key => $value) {
            if (is_string($value)) {
                $substituted = $this->substituteVariables($value, $input);
                // Try to convert numeric strings
                if (is_numeric($substituted) && !str_contains($substituted, '.')) {
                    $processed[$key] = (int)$substituted;
                } elseif (is_numeric($substituted)) {
                    $processed[$key] = (float)$substituted;
                } elseif ($substituted === 'true') {
                    $processed[$key] = true;
                } elseif ($substituted === 'false') {
                    $processed[$key] = false;
                } elseif ($substituted === 'null' || $substituted === 'NULL') {
                    $processed[$key] = null;
                } else {
                    $processed[$key] = $substituted;
                }
            } elseif (is_array($value)) {
                // Recursively process nested arrays (e.g., JSON objects)
                $processed[$key] = json_encode($this->processDatastoreData($value, $input));
            } else {
                $processed[$key] = $value;
            }
        }
    ...
    /**
     * Execute a lightweight LLM API call
     *
     * Supports both Anthropic API and local Ollama (which speaks Anthropic format).
     * This is for simple prompt->response tasks, not full agent sessions.
     *
     * Config options:
     *   provider     - 'anthropic' (default) or 'ollama'
     *   base_url     - API base URL (default: https://api.anthropic.com for anthropic, http://localhost:11434 for ollama)
     *   model        - Model name (e.g., 'claude-3-haiku-20240307', 'qwen3-coder:30b')
     ...
     *   json_output  - If true, parse response as JSON and return as output object
     *
     * Example config:
     *   {
     *     "provider": "ollama",
     ...
     *     "system_prompt": "You are a classifier. Respond with JSON only.",
     *     "json_output": true
     *   }
     */
    private function executeLlmCall(array $config, array $input, int $timeout): array {
        $provider = $config['provider'] ?? 'anthropic';
        $model = $config['model'] ?? 'claude-3-haiku-20240307';
        $maxTokens = (int)($config['max_tokens'] ?? 1024);
        $temperature = (float)($config['temperature'] ?? 0);
        $jsonOutput = !empty($config['json_output']);
        ...
        // Substitute variables in prompts
        $prompt = $this->substituteVariables($config['prompt'] ?? '', $input);
        ...
        if (empty($prompt)) {
            return ['success' => false, 'error' => 'LLM call requires a prompt'];
        }

        $this->log('info', "Executing LLM call", [
            'provider' => $provider,
            'model' => $model,
            'base_url' => $baseUrl,
        ...
        try {
            ...
            // Build request body (Anthropic format - Ollama supports this natively)
            $requestBody = [
                'model' => $model,
                'max_tokens' => $maxTokens,
                'messages' => $messages,
            ...
            if ($provider === 'anthropic') {
                // Use AnthropicKeyService to get decrypted key
                $memberId = $this->run->member_id ?? 1;
                $apiKey = AnthropicKeyService::getApiKey((int)$memberId, $model);
            ...
            if ($curlError) {
                return ['success' => false, 'error' => "LLM API error: {$curlError}"];
            }

            if ($httpCode !== 200) {
                $this->log('error', "LLM API returned non-200", [
                    'http_code' => $httpCode,
                    'response' => substr($response, 0, 500)
                ]);
                return ['success' => false, 'error' => "LLM API returned HTTP {$httpCode}: " . substr($response, 0, 200)];
            }
            ...
            } elseif (isset($responseData['content']) && is_string($responseData['content'])) {
            ...
            if ($jsonOutput) {
                ...
                if (json_last_error() === JSON_ERROR_NONE) {
                    ...
                    $this->log('warning', "Failed to parse LLM response as JSON", [
                        'error' => json_last_error_msg(),
                        'text' => substr($text, 0, 200)
                    ]);
                    // Return raw text as fallback
                    $output = ['raw_response' => $text, 'parse_error' => json_last_error_msg()];
                }
            }
            ...
            return [
                'success' => true,
                'output' => $output,
                'stdout' => is_string($output) ? $output : json_encode($output, JSON_PRETTY_PRINT),
                'model' => $model,
                'usage' => $responseData['usage'] ?? null
            ];

        } catch (\Exception $e) {
            $this->log('error', "LLM call failed: " . $e->getMessage());
            return [
                'success' => false,
                'error' => "LLM call exception: " . $e->getMessage()
            ];
        }
    }

    /**
     * Execute Shopify Liquid evaluation using Shopify CLI theme console
     *
     * Config options:
     *   connection_id - Shopify connection ID (required). Supports variable substitution.
     *   liquid_code   - Liquid template code to evaluate (required). Supports variable substitution.
     *   url           - Page context URL (optional). E.g., /products/my-product for product Liquid objects.
     *
     * Requires:
     *   - Shopify CLI installed (`shopify` command available in PATH)
     *   - Custom app token with read_themes, write_themes, unauthenticated_read_content scopes
     *
     * Output:
     *   {
     *     "success": true,
     *     "result": "evaluated liquid output as text",
     *     "shop": "mystore.myshopify.com",
     *     "url": "/products/test-product"
     *   }
     */
    private function executeShopifyLiquidEval(array $config, array $input): array {
        require_once __DIR__ . '/ConnectionsService.php';

        $connectionId = $config['connection_id'] ?? null;
        $liquidCode = $config['liquid_code'] ?? '';
        $url = $config['url'] ?? '';
        ...
        if (empty($liquidCode)) {
            return ['success' => false, 'error' => 'liquid_code is required'];
        }

        // Substitute variables in liquid code and URL
        $liquidCode = $this->substituteVariables($liquidCode);
        $url = $this->substituteVariables($url);

        $this->log('info', "Executing Shopify Liquid evaluation", [
            'connection_id' => $connectionId,
            'code_length' => strlen($liquidCode),
            'url' => $url ?: '(none)'
        ]);

        try {
            // Load Shopify connection (supports unified connections table)
            $conn = ConnectionsService::getShopifyConnection($connectionId);
            if (!$conn) {
                return ['success' => false, 'error' => "Shopify connection not found: {$connectionId}"];
            }

            $shop = $conn->external_eid ?? null;
            $accessToken = $conn->access_token ?? null;

            if (!$shop || !$accessToken) {
                return ['success' => false, 'error' => 'Shopify connection missing shop domain or access token'];
            }

            // Check if shopify CLI is available
            $cliCheck = shell_exec('which shopify 2>/dev/null');
            if (empty($cliCheck)) {
                return [
                    'success' => false,
                    'error' => 'Shopify CLI not installed. Install with: npm install -g @shopify/cli'
                ];
            }

            // Build shopify theme console command
            $escapedShop = escapeshellarg($shop);
            $escapedToken = escapeshellarg($accessToken);
            $escapedCode = escapeshellarg($liquidCode);

            $cmd = "shopify theme console --store {$escapedShop} --password {$escapedToken}";
            ...
            // Pass Liquid code via stdin
            $cmd .= " <<< {$escapedCode} 2>&1";

            $this->log('debug', "Executing Shopify CLI command", ['shop' => $shop]);
            ...
            if ($output === null) {
                return [
                    'success' => false,
                    'error' => 'Failed to execute shopify theme console command'
                ];
            }

            // Clean up the output (remove CLI prompts, color codes, etc.)
            $result = trim($output);

            // Check for common error patterns
            if (stripos($result, 'error') !== false || stripos($result, 'failed') !== false) {
                $this->log('warning', "Liquid evaluation returned error-like output", ['output' => substr($result, 0, 200)]);
                // Still return success but log the warning - Liquid errors are valid results
            }
            ...
            $this->log('error', "Shopify Liquid evaluation failed: " . $e->getMessage());
            return [
                'success' => false,
                'error' => "Liquid evaluation exception: " . $e->getMessage()
            ];
        }
    }
    ...
    private function executeMailgun(array $config, array $input): array {
        ...
        try {
            ...
            if ($contentType === 'markdown') {
                ...
                $htmlBody = '<pre style="font-family: sans-serif; white-space: pre-wrap;">' . h($body) . '</pre>';
    ...
    /**
     * Execute a LinkedIn enrichment step
     *
     * Modes:
     *   enrich_contacts  - Batch enrich existing CRM contacts with LinkedIn data
     *   search_companies - Search for companies by keywords
     *   search_people    - Search for people by keywords
     */
    private function executeLinkedInEnrich(array $config, array $input, int $timeout): array
    {
        require_once __DIR__ . '/LinkedInBrowserService.php';

        $mode = $config['mode'] ?? 'enrich_contacts';
        $country = $config['country'] ?? 'US';
        $maxResults = (int) ($config['max_results'] ?? 10);
        $delayMs = max(1000, (int) ($config['delay_ms'] ?? 3000));
        $productOnly = !empty($config['product_only']);

        $workspace = $this->context['_workspace'] ?? $_SERVER['WORKSPACE'] ?? 'default';

        $linkedin = new LinkedInBrowserService($workspace, $this->logger, [
            'daily_limit' => 100,
        ]);
        ...
        if (empty($session['valid'])) {
            return [
                'success' => false,
                'error' => 'LinkedIn session is not valid. Re-authenticate via VNC.',
                'session' => $session,
            ];
        }

        $this->log('info', "LinkedIn enrichment: mode={$mode}, workspace={$workspace}");

        switch ($mode) {
            case 'search_companies':
                $keywords = $this->substituteVariables($config['keywords'] ?? '');
                if (empty($keywords)) {
                    return ['success' => false, 'error' => 'Keywords are required for company search'];
                }

                $result = $linkedin->searchCompanies($keywords, [
                    'country' => $country,
                    'count' => min($maxResults, 10),
            ...
            case 'search_people':
                $keywords = $this->substituteVariables($config['keywords'] ?? '');
                if (empty($keywords)) {
                    return ['success' => false, 'error' => 'Keywords are required for people search'];
                }

                $result = $linkedin->searchPeople($keywords, [
                    'country' => $country,
                    'count' => min($maxResults, 10),
                    'company' => $this->substituteVariables($config['company'] ?? ''),
                ]);
            ...
            case 'enrich_contacts':
                $batchLimit = (int) ($config['batch_limit'] ?? 10);
                $scoreBoost = (int) ($config['score_boost'] ?? 15);

                // Find contacts needing LinkedIn enrichment
                $contacts = Bean::find('crmcontact',
                    '(linkedin_url IS NULL OR linkedin_url = ?) AND company_name IS NOT NULL AND company_name != ? AND enrichment_status = ? ORDER BY lead_score DESC LIMIT ?',
                    ['', '', 'enriched', $batchLimit]
                );
                ...
                foreach ($contacts as $contact) {
                    ...
                    if (!empty($companyResult['found']) && !empty($companyResult['match'])) {
                        $match = $companyResult['match'];
                        $contact->linkedinUrl = $match['linkedinUrl'] ?? '';

                        // Merge LinkedIn data into enrichment JSON
                        ...
                        // Boost lead score
                        $currentScore = (int) $contact->leadScore;
                        $contact->leadScore = min(100, $currentScore + $scoreBoost);
                        ...
                        $touch->summary = "LinkedIn enrichment: found company '{$match['name']}' ({$match['followers']})";
                        ...
                        $results[] = [
                            ...
                            'linkedin_match' => $match['name'],
                            'score_boost' => $scoreBoost,
                            'new_score' => $contact->leadScore,
                        ];
                    } else {
                        $failed++;
                        $results[] = [
                            ...
                            'company' => $contact->companyName,
                            'linkedin_match' => null,
                            'error' => $companyResult['reason'] ?? 'No match found',
                        ];
                    }
            ...
            default:
                return ['success' => false, 'error' => "Unknown LinkedIn mode: {$mode}"];
        }
    }
}
