    ...
    public function edit($params = []) {
        ...
        if ($request->method === 'POST') {
            ...
            $data = [
                ...
                // Execution mode: NULL = local runner, integer = connections.id (anthropic type)
        ...
        // Fetch API keys for execution mode dropdown (Enterprise feature) - from connections table
        $anthropicKeys = [];
        if ($this->member->isEnterprise()) {
            try {
                require_once __DIR__ . '/../services/EncryptionService.php';
                $keys = Bean::findAll('connections', ' connector_type = ? ORDER BY connection_name ASC ', ['anthropic']);
                foreach ($keys as $key) {
                    $decrypted = \app\services\EncryptionService::decrypt($key->access_token);
