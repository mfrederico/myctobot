...
/**
 * Detect new *_id bean properties that may store string values.
 *
 * RedBeanPHP auto-types columns ending in _id as INT foreign keys.
 * If the value is a string (e.g. Stitch screen IDs, external IDs),
 * storing it silently fails or truncates. Use _eid suffix instead.
 *
 * Known safe _id columns (legitimate integer FKs) are allowlisted.
 *
 * @param string $content PHP code to check
 * @return array Blocking issues
 */
function findStringIdColumns(string $content): array
{
    $issues = [];

    // Known safe integer FK columns - these are legitimate _id references
    $safeFkColumns = [
        'member_id', 'members_id', 'boards_id', 'jiraboards_id',
        'pipelines_id', 'pipelinesteps_id', 'pipelineruns_id', 'pipelinestepruns_id',
        'repoconnections_id', 'aidevjobs_id', 'aiagents_id', 'runners_id',
        'sshkey_id', 'apikeys_id', 'detachableapps_id', 'ceodirectives_id',
        'shopifyconnections_id', 'connections_id', 'workstation_id', 'workstations_id',
        'tenantapps_id', 'knowledgebase_id', 'knowledgebases_id',
        'pipeline_id', 'step_id', 'agent_id', 'board_id', 'repo_id',
        'parent_id', 'created_by_member_id',
    ];

    // Pattern: $bean->something_id = $stringVar or $bean->something_id = trim(...)
    // or $bean->something_id = 'literal string'
    if (preg_match_all(
        '/\$(\w+)->(\w+_id)\s*=\s*(.+?);/m',
        $content,
        $matches,
        ...
        foreach ($matches as $match) {
            ...
            // Skip known safe FK columns
            if (in_array($column, $safeFkColumns)) {
                continue;
            }

            $value = trim($match[3]);

            // Detect string assignments: literal strings, trim(), string functions
            $looksLikeString = false;
            if (preg_match("/^['\"](?!\\d+['\"]$)/", $value)) {
                // Literal string that's not purely numeric like '42'
                $looksLikeString = true;
            } elseif (preg_match('/^trim\s*\(/', $value)) {
                $looksLikeString = true;
            } elseif (preg_match('/^(?:str_replace|strtolower|strtoupper|substr|sprintf)\s*\(/', $value)) {
                $looksLikeString = true;
            }

            if ($looksLikeString) {
                $suggested = preg_replace('/_id$/', '_eid', $column);
                $issues[] = "STRING VALUE IN _id COLUMN: \$bean->{$column} is assigned a string value. "
                    . "RedBeanPHP auto-types _id columns as INT foreign keys — string values will silently fail!\n"
                    . "   Rename to: {$suggested} (external ID)\n"
                    . "   Or if this IS an integer FK, cast: (int) {$value}";
            }
        }
    }
...
function validatePhpCode(string $content, string $filePath = ''): array
{
    ...
    $blockingIssues = array_merge($blockingIssues, findStringIdColumns($content));
