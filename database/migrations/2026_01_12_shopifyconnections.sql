...
CREATE TABLE IF NOT EXISTS `shopifyconnections` (
    ...
    `shared` TINYINT(1) DEFAULT 0,               -- Share with workspace members
...
-- Migration for existing tables (add shared column if missing)
-- This is safe to run multiple times
ALTER TABLE `shopifyconnections` ADD COLUMN IF NOT EXISTS `shared` TINYINT(1) DEFAULT 0 AFTER `enabled`;
ALTER TABLE `shopifyconnections` ADD INDEX IF NOT EXISTS `idx_member` (`created_by_member_id`);
ALTER TABLE `shopifyconnections` ADD INDEX IF NOT EXISTS `idx_shared` (`shared`);
