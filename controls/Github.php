...
class Github extends BaseControls\Control {
    ...
    /**
     * Main GitHub page - shows repo management
     */
    ...
    /**
     * Start GitHub OAuth flow - delegates to generic Connections controller
     */
    public function connect() {
        Flight::redirect('/connections/connect/github');
    }

    /**
     * Handle GitHub OAuth callback - forwards to generic Connections controller
     */
    public function callback() {
        $queryString = $_SERVER['QUERY_STRING'] ?? '';
        Flight::redirect('/connections/callback/github' . ($queryString ? '?' . $queryString : ''));
    }
    ...
    public function addrepo() {
        ...
        $connectionId = Flight::request()->data->connection_id ?? null;
        ...
        try {
            ...
            // Link to specific GitHub connection if provided
            if ($connectionId) {
                $repo->connections_id = (int)$connectionId;
            }
            ...
            $webhookError = null;
            try {
                // Prefer the linked connection if specified
                $conn = null;
                if ($connectionId) {
                    $conn = Bean::load('connections', (int)$connectionId);
                    if (!$conn->id || $conn->connector_type !== 'github') {
                        $conn = null;
                    }
                }
                if (!$conn) {
                ...
                if ($conn && !empty($conn->access_token)) {
                    ...
                    // Use workspace URL so webhook callbacks go to the correct workspace
                    $webhookUrl = \app\services\SiteConfig::getWorkspaceUrl() . '/webhook/github';

                    // Check if webhook already exists
                    $existingHook = $github->findWebhook($owner, $repoName, $webhookUrl);
                    if ($existingHook) {
                        // Update existing webhook with our new secret so signatures match
                        $github->updateWebhook($owner, $repoName, (int) $existingHook['id'], $webhookUrl, $webhookSecret);
                        $webhookCreated = true;
                        $repo->webhook_uid = $existingHook['id'];
                        $this->logger->info('Found and updated existing webhook', ['hook_id' => $existingHook['id'], 'url' => $webhookUrl]);
                    } else {
                        // Create webhook
    ...
    public function disconnect() {
        ...
        // Remove GitHub connections from unified connections table
        $githubConns = Bean::find('connections', 'connector_type = ?', ['github']);
        foreach ($githubConns as $conn) {
            Bean::trash($conn);
        }

        // Also clean up legacy enterprisesettings if they exist
    ...
    public function repolist() {
        ...
        try {
            ...
            // Check if GitHub is connected (from unified connections table)
            $githubConn = Bean::findOne('connections', 'connector_type = ? ORDER BY id ASC', ['github']);
            $githubToken = $githubConn ? EncryptionService::decrypt($githubConn->access_token) : null;

            // Get all GitHub connections for account selector
            $githubConnections = Bean::find('connections', 'connector_type = ? ORDER BY id ASC', ['github']);
    ...
    public function disconnectrepo($params = []) {
        ...
        try {
            ...
            if (!empty($repo->webhook_uid) && !empty($repo->repo_owner) && !empty($repo->repo_name)) {
                try {
                    // Prefer repo's linked connection
                    $conn = null;
                    if (!empty($repo->connections_id)) {
                        $conn = Bean::load('connections', (int)$repo->connections_id);
    ...
    public static function getTokenStatic(string $owner, string $repo, int $memberId): ?string {
        ...
        // 2. Try linked connection (connections_id FK)
        if (empty($githubToken) && !empty($repoConnection->connections_id)) {
            $conn = Bean::load('connections', (int)$repoConnection->connections_id);
            if ($conn->id && !empty($conn->access_token)) {
                $githubToken = EncryptionService::decrypt($conn->access_token);
            }
        }

        // 3. Generic fallback - any GitHub connection for this member
        if (empty($githubToken)) {
            $conn = Bean::findOne('connections', 'connector_type = ? AND (member_id = ? OR shared = 1) ORDER BY id ASC', ['github', $memberId]);
            if ($conn && !empty($conn->access_token)) {
                $githubToken = EncryptionService::decrypt($conn->access_token);
            }
        }
