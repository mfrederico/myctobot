...
class UserDatabaseService {
    ...
    // ==================== Anthropic Keys (via unified connections table) ====================

    /**
     * Get all Anthropic API keys from the unified connections table
     * @return array
     */
    public static function getAnthropicKeys(): array {
        return array_values(Bean::findAll('connections', ' connector_type = ? ORDER BY created_at DESC ', ['anthropic']));
    }

    /**
     * Get an Anthropic key by ID from the unified connections table
     * @param int $id Connection ID
     * @return \RedBeanPHP\OODBBean|null
     */
    public static function getAnthropicKey(int $id) {
        $key = Bean::load('connections', $id);
        return ($key->id && $key->connector_type === 'anthropic') ? $key : null;
    }

    /**
     * Add an Anthropic API key to the unified connections table
    ...
    public static function addAnthropicKey(string $name, string $encryptedKey, string $model): int {
        ...
        $key->access_token = $encryptedKey;
        $key->metadata_json = json_encode(['model' => $model]);
        $key->member_id = $_SESSION['member_id'] ?? null;
        $key->created_at = date('Y-m-d H:i:s');
        return Bean::store($key);
    }

    /**
     * Delete an Anthropic key from the unified connections table
     * @param int $id Connection ID
     * @return bool
     */
