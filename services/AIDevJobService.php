...
class AIDevJobService {
    ...
    public function triggerJob(int $memberId, string $issueKey, string $cloudId, ?int $boardId = null, ?int $repoId = null, ?string $workspace = null, bool $useOrchestrator = false, string $projectType = self::PROJECT_TYPE_JIRA): array {
        try {
            ...
            // === API execution path - get key and model from connections table ===
            $keyBean = Bean::load('connections', $boardKeyId);

            if (!$keyBean || !$keyBean->id || $keyBean->connector_type !== 'anthropic' || empty($keyBean->access_token)) {
                return ['success' => false, 'error' => 'Selected API key not found or invalid'];
            }

            $apiKey = EncryptionService::decrypt($keyBean->access_token);
            $metadata = json_decode($keyBean->metadata_json ?: '{}', true);
    ...
    private function spawnGitHubLocalRunner(int $memberId, string $issueKey, int $repoId, ?string $workspace = null): array {
        ...
        // Create job record (use null for boardId since GitHub doesn't have boards)
        $jobId = AIDevStatusService::createJob($memberId, null, $issueKey, $repoId, null, 'github');
     ...
     * @param int $memberId Member ID
     * @param string $issueKey Issue key
     * @param int|null $boardId Board ID (null for non-Jira projects)
    ...
    private function cleanupGitHubLabels($job, int $memberId, array $result): array {
        ...
        if (empty($githubToken)) {
            $conn = Bean::findOne('connections', 'connector_type = ? AND (member_id IS NOT NULL OR shared = 1) ORDER BY id ASC', ['github']);
                ...
                if ($shopifyConn && !isset($cleanedStores[$shopifyConn->id])) {
                    try {
                        ...
                        $this->logger->info('Cleaned up Shopify dev themes', [
                            'issue_key' => $issueKey,
                            'shop_domain' => $shopifyConn->external_eid,
                            'themes_deleted' => $deleted
                        ]);
                    } catch (\Exception $e) {
                        $results['errors'][] = "Shopify cleanup ({$shopifyConn->external_eid}): " . $e->getMessage();
                    }
                }
            }
    ...
    public function getShopifySettingsForPayload(int $memberId, ?int $repoId = null): array {
        ...
        try {
            ...
            if ($shopifyClient->isConnected()) {
                ...
                // Get additional settings from connections table if using multi-store
                if ($shopifyConn) {
                    $metadata = json_decode($shopifyConn->metadata_json ?? '{}', true);
                    if (!empty($metadata['storefront_password'])) {
                        $settings['storefront_password'] = EncryptionService::decrypt($metadata['storefront_password']);
                    }
                    $settings['verify_with_playwright'] = (bool)($metadata['verify_with_playwright'] ?? false);
                } else {
