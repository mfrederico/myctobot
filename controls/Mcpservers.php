...
class Mcpservers extends BaseControls\Control {
    ...
    public function delete($params = []) {
        ...
        // Owner can delete their own servers; admins can delete any server (including ownerless builtins)
        $isOwner = $server->member_id !== null && (int)$server->member_id === (int)$this->member->id;
    ...
    private function getPresets(): array {
        ...
        return [
            ...
            'myctobot-jobs' => [
                ...
                'headers' => ['Authorization' => 'Bearer ' . ($mcpApiKey ?: 'tk_CREATE_KEY_AT_APIKEYS_WITH_MCP_SCOPE')],
                'env' => [],
                'note' => $mcpApiKey ? 'HTTP endpoint for AI Dev job status callbacks' : 'Create an API key at /apikeys with mcp:* scope'
            ],
            'jira' => [
                'name' => 'jira',
                ...
                'headers' => ['Authorization' => 'Bearer ' . ($mcpApiKey ?: 'tk_CREATE_KEY_AT_APIKEYS_WITH_MCP_SCOPE')],
                'env' => [],
                'note' => $mcpApiKey ? 'HTTP endpoint for Jira tools' : 'Create an API key at /apikeys with mcp:* scope'
            ],
            'github' => [
                'name' => 'github',
