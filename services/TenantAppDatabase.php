...
class TenantAppDatabase
{
    ...
    /**
     * Ensure the database connection is alive, reconnecting if needed.
     *
     * Long-lived processes (OpenSwoole servers, daemons) may lose their
     * MySQL connection due to wait_timeout. Call this before each request
     * to transparently reconnect when the connection has gone away.
     */
    public static function ensureConnection(): void
    {
        if (!self::$initialized || self::$currentWorkspace === null) {
            return;
        }

        try {
            // Lightweight ping — if the connection is dead this will throw
            R::getCell('SELECT 1');
        } catch (\Throwable $e) {
            // Connection lost — force reconnect
