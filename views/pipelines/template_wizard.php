<div class="container py-4" data-assist-page="pipelines/template_wizard" data-assist-purpose="Create new pipeline using template wizard">
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb mb-3">
            <li class="breadcrumb-item"><a href="/pipelines">Pipelines</a></li>
            <li class="breadcrumb-item"><a href="/pipelines?tab=templates">Templates</a></li>
            ...
            <div class="card mb-4">
                <div class="card-body">
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            <h4 class="mb-1"><?= h($template['name']) ?></h4>
                            <p class="text-muted mb-0"><?= h($template['description']) ?></p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card">
                ...
                <div class="card-footer d-flex justify-content-between">
                    ...
                    <button type="button" id="btnCreate" class="btn btn-success d-none" onclick="createProject()">
                        <i class="bi bi-check-lg"></i> Create Pipeline
                    </button>
                </div>
            </div>
        </div>
...
function renderSummaryStep(step) {
    let html = `
        <div class="alert alert-info">
            <i class="bi bi-info-circle"></i> Review your configuration before creating the pipeline.
        </div>
...
async function saveStepData() {
    ...
    try {
        const response = await fetch('/pipelines/templatewizardstep', {
            method: 'POST',
            headers: {
...
async function createProject() {
    ...
    try {
        const response = await fetch('/pipelines/templatefinalize', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-Requested-With': 'XMLHttpRequest'
        ...
        if (result.success) {
            if (result.data.pipeline_id) {
                window.location.href = '/pipelines/edit/' + result.data.pipeline_id;
            } else {
                window.location.href = '/pipelines?tab=templates';
            }
        } else {
