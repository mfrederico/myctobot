<div class="container py-4" data-assist-page="reviewboard" data-assist-purpose="Kanban board for reviewing AI Developer jobs">
    ...
    <div class="alert alert-info">
        ...
        <hr>
        <p class="mb-0">
            Send a directive to <code><?= h($_SESSION['workspace_slug'] ?? 'your-workspace') ?>@myctobot.ai</code> to get started.
        </p>
    </div>
    <?php else: ?>
    ...
    <ul class="nav nav-tabs mb-3" id="projectTabs" role="tablist">
        ...
        ?>
        <li class="nav-item" role="presentation">
            <button class="nav-link" id="tab-<?= $projectSlug ?>" data-project-filter="<?= h($projectName) ?>" type="button" role="tab">
                <?= h($projectName) ?>
        ...
        <div class="col-md-3">
            <div class="card h-100">
                ...
                <div class="card-body kanban-column" data-status="pending_review">
                    ...
                    <div class="card mb-2 kanban-card" data-story-id="<?= $story->story_uid ?>" data-project="<?= h($project->name) ?>">
                            ...
                            <div class="d-flex justify-content-between align-items-start mb-1">
                                <div class="form-check">
                                    <input class="form-check-input pending-select-checkbox" type="checkbox"
                                           ...
                                           data-issue-key="<?= h($story->jira_issue_key ?? '') ?>"
                                           data-title="<?= h($story->title) ?>"
                                           onchange="updatePendingSelection()">
                                </div>
                                <small class="text-muted text-truncate" style="max-width: 40%;" title="<?= h($project->name . ' / ' . $epic->title) ?>">
                                    <?= h($project->name) ?>
                                </small>
                                <div>
                                    <?php
                                    ...
                                    ?><span class="badge bg-light text-dark" title="<?= h($story->jira_issue_key ?? 'Story #' . $story->id) ?>">#<?= $issueNum ?></span>
                    ...
                    <div class="card mb-2 kanban-card" data-story-id="<?= $story->story_uid ?>" data-project="<?= h($project->name) ?>">
                        <div class="card-body p-2">
                            ...
                            <small class="text-success d-block mb-2">
                                <i class="bi bi-link-45deg"></i> <?= h($story->jira_issue_key) ?>
                            </small>
                            <?php endif; ?>
                            ...
                            <div class="d-flex justify-content-between align-items-center">
                                ...
                                <div class="btn-group btn-group-sm">
                                    ...
                                    <button class="btn btn-success" onclick="startJob('<?= h($story->jira_issue_key) ?>')" title="Start Job">
                                        <i class="bi bi-play-fill"></i>
                                    </button>
                                    <?php endif; ?>
                                    <?php if ($story->jira_issue_key): ?>
                                    <button class="btn btn-outline-secondary" onclick="viewJobResults('<?= h($story->jira_issue_key) ?>')" title="View Details">
                                        <i class="bi bi-eye"></i>
                                    </button>
                                    <?php endif; ?>
                                </div>
        ...
        <div class="col-md-3">
            <div class="card h-100">
                ...
                <div class="card-body kanban-column" data-status="in_progress">
                    ...
                    <div class="card mb-2 kanban-card border-primary" data-story-id="<?= $story->story_uid ?>" data-project="<?= h($project->name) ?>">
                        <div class="card-body p-2">
                            <div class="d-flex justify-content-between align-items-start mb-1">
                                <small class="text-muted text-truncate" style="max-width: 60%;">
                                    <?= h($project->name) ?>
                            ...
                            <small class="text-muted d-block mb-2">
                                <i class="bi bi-activity"></i> <?= h($job['current_step'] ?? 'Processing...') ?>
                            </small>
                            ...
                            <?php endif; ?>
                            <?php endif; ?>
                            <button class="btn btn-sm btn-outline-info w-100" onclick="viewJobResults('<?= h($story->jira_issue_key) ?>')">
        ...
        <div class="col-md-3">
            <div class="card h-100">
                ...
                <div class="card-body kanban-column" data-status="complete">
                    ...
                    <?php $story = $card['story']; $project = $card['project']; $job = $card['job']; ?>
                    <div class="card mb-2 kanban-card border-success" data-story-id="<?= $story->story_uid ?>" data-project="<?= h($project->name) ?>" data-issue-key="<?= h($story->jira_issue_key ?? '') ?>">
                        <div class="card-body p-2">
                            <div class="d-flex justify-content-between align-items-start mb-1">
                                <div class="form-check">
                            ...
                            <div class="btn-group btn-group-sm w-100">
                                ...
                                <a href="<?= h($job['pr_url']) ?>" target="_blank" class="btn btn-outline-success">
                                    <i class="bi bi-git"></i> View PR
                                </a>
                                <?php endif; ?>
                                <button class="btn btn-outline-secondary" onclick="viewJobResults('<?= h($story->jira_issue_key) ?>')">
    ...
    <div class="card mt-3 border-danger">
        ...
        <div class="card-body">
            <div class="row g-2">
                ...
                <?php $story = $card['story']; $project = $card['project']; $job = $card['job']; ?>
                <div class="col-md-3 failed-card-col" data-project="<?= h($project->name) ?>">
                    <div class="card border-danger kanban-card" data-story-id="<?= $story->story_uid ?>" data-project="<?= h($project->name) ?>">
                        <div class="card-body p-2">
                            <div class="d-flex justify-content-between align-items-start mb-1">
                                <small class="text-muted text-truncate" style="max-width: 50%;">
                                    <?= h($project->name) ?>
                            ...
                            <small class="text-danger d-block mb-2 text-truncate" title="<?= h($job['error_message']) ?>">
                                <i class="bi bi-exclamation-triangle"></i> <?= h(substr($job['error_message'], 0, 50)) ?>...
                            </small>
                            <?php endif; ?>
                            <div class="btn-group btn-group-sm w-100">
                                <?php if ($job && !empty($job['job_uid'])): ?>
                                <button class="btn btn-warning" onclick="retryJob('<?= h($job['job_uid']) ?>')" title="Retry">
                                    <i class="bi bi-arrow-repeat"></i> Retry
                                </button>
                                <?php endif; ?>
                                <button class="btn btn-outline-secondary" onclick="viewJobResults('<?= h($story->jira_issue_key) ?>')" title="View Logs">
