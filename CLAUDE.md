...
## RULE #3: NEVER CREATE EXPLICIT ROUTE FILES

**Use DefaultRoute auto-routing. NEVER create files in `routes/` for new endpoints.**

The DefaultRoute handler automatically routes URLs to controllers:
- URL `/api/health` → `controls/Api.php` → `health()` method
- URL `/mcp/tools` → `controls/Mcp.php` → `tools()` method

**To add a new endpoint:**
1. Add a method to the controller (lowercase name)
2. Add an `authcontrol` entry for permissions
3. Done. No route file needed.

```php
// CORRECT - Just add a method to the controller
class Api {
    public function newfeature($params = []) {
        // DefaultRoute handles routing automatically
    }
}
```

```php
// WRONG - Never create explicit routes!
// routes/api.php
Flight::route('GET /api/newfeature', function() { ... });
```

**Why this matters:**
- DefaultRoute handles auth/permissions via `authcontrol` table
- Explicit routes bypass permission checks (security risk)
- Explicit routes duplicate logic that already exists
- Explicit routes require maintenance when patterns change

**For stateless API endpoints:**
- Set `authcontrol` level to PUBLIC (101)
- Validate Bearer token in the controller method via `ApiAuthService::authenticate()`

See `FLIGHTPHP_README.md` for detailed routing patterns.
...
URL segments after `/controller/method/` are available as a numeric splat array on `$params[]`:

```
URL: /controller/method/seg0/seg1/seg2/seg3
                         ↓     ↓     ↓     ↓
                    $params[0] [1]   [2]   [3]
```

```php
// URL: /schedules/edit/42
public function edit($params = []) {
    $id = (int) ($params[0] ?? 0);  // "42"
}

// URL: /pipelines/runs/5/step/send_email
public function runs($params = []) {
    $pipelineId = (int) ($params[0] ?? 0);  // "5"
    $action     = $params[1] ?? null;        // "step"
    $stepName   = $params[2] ?? null;        // "send_email"
}
```

**Legacy helpers still work:** `$this->opId()` = `$params[0]`, `$this->opType()` = `$params[1]`.
**Always cast to `(int)` when loading beans** — URL segments and JSON values are strings, and `Bean::load()` requires int.
...
> **See RULE #3 above.** This is a common mistake - NEVER create `routes/*.php` files for new endpoints.

**The DefaultRoute handler automatically manages routing, auth, and permissions.**
Creating explicit route files is ALWAYS wrong. There are NO exceptions.
...
**NEVER do this (violates RULE #3):**
```php
// routes/mcp.php - DON'T create explicit routes!
// This bypasses DefaultRoute's auth/permission checks!
...
**If you find yourself creating a route file, STOP and ask:**
1. Did I add a method to the controller? (Required)
2. Did I add an authcontrol entry? (Required for permissions)
3. Is the method name lowercase? (Required for auto-routing)

If yes to all three, the endpoint works. No route file needed.
...
> **For detailed routing patterns and examples, see `FLIGHTPHP_README.md`.**
...
### MCP Pipeline Management Tools

The `/mcp/pipelines` endpoint exposes tools for managing pipelines and steps:

| Tool | Description |
|------|-------------|
| `list_pipelines` | List all pipelines (id, name, slug, step_count, run_count, etc.) |
| `get_pipeline` | Get pipeline details by ID or slug, optionally with steps |
| `set_pipeline` | Create or update a pipeline (upsert by ID or slug) |
| `delete_pipeline` | Delete a pipeline and all its steps (requires confirm=true) |
| `get_step` | Get step configuration by ID or pipeline_slug + step_name |
| `set_step` | Create or update a step (upsert) |
| `delete_step` | Delete a step from a pipeline |
| `run_pipeline` | Trigger a pipeline run, returns run_id for tracking |
| `export_pipeline` | Export pipeline as JSON (for backup/duplicate/transfer) |
| `import_pipeline` | Import pipeline from JSON export (for duplicate/transfer) |
| `list_runs` | List pipeline runs with optional filters (pipeline, status) |
| `get_run` | Get detailed run info including step statuses |
| `cancel_run` | Cancel a running pipeline |
| `create_schedule` | Create a recurring schedule for a pipeline |
| `list_schedules` | List all recurring schedules |
| `get_schedule` | Get schedule details including next run times |
| `update_schedule` | Update an existing schedule |
| `delete_schedule` | Delete a recurring schedule |
| `get_pipeline_components` | Get available step types, schemas, and integrations |

**Quick step update example:**
```json
// Get current config
{"method": "tools/call", "params": {"name": "get_step", "arguments": {
  "pipeline_slug": "contact-classifier",
  "step_name": "classify_ai"
}}}

// Update config
{"method": "tools/call", "params": {"name": "set_step", "arguments": {
  "pipeline_slug": "contact-classifier",
  "step_name": "classify_ai",
  "config": {"provider": "ollama", "model": "qwen3-coder:30b"}
}}}
```

**Addressing patterns:**
- By ID: `step_id=129` or `pipeline_id=22`
- By slug + name: `pipeline_slug="contact-classifier"` + `step_name="classify_ai"`
...
ssh claudeuser@shard.example.com
...
php scripts/cron/cron-analysis.php --script --secret=KEY --member=3 --board=1 --workspace=gwt

# Run digest cron (processes all workspaces automatically)
php scripts/cron/cron-digest.php --script --verbose
...
$cmd = sprintf(
    'php scripts/cron/cron-analysis.php --script --secret=%s --member=%d --board=%d%s',
