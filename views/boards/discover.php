<div class="container py-4" data-assist-page="boards/discover" data-assist-purpose="Discover and add new Jira boards from connected sites">
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="card mb-4">
                <div class="card-header bg-success text-white d-flex justify-content-between align-items-center">
                    <span>
                        ...
                        <strong><?= h($site->site_name) ?></strong>
                        <small class="ms-2"><?= h($site->site_url) ?></small>
                    </span>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <code><?= h($board['project_key']) ?></code>
                                        <br>
                                        <small class="text-muted"><?= h($board['project_name'] ?? '') ?></small>
                                        <?php else: ?>
                                    ...
                                    <td>
                                        ...
                                        <form method="POST" action="/boards/add" class="d-inline">
                                            <input type="hidden" name="cloud_uid" value="<?= h($site->cloud_uid) ?>">
                                            <input type="hidden" name="board_id" value="<?= $board['id'] ?>">
                                            <input type="hidden" name="board_name" value="<?= h($board['name']) ?>">
                                            <input type="hidden" name="board_type" value="<?= h($board['type']) ?>">
                                            <input type="hidden" name="project_key" value="<?= h($board['project_key'] ?? '') ?>">
                                            <input type="hidden" name="project_name" value="<?= h($board['project_name'] ?? '') ?>">
                                            <input type="hidden" name="site_name" value="<?= h($site->site_name) ?>">
                                            <input type="hidden" name="site_url" value="<?= h($site->site_url) ?>">
