...
class Apikeys extends BaseControls\Control {
    ...
    private function formatKey($key): array {
        // Load member info
        $member = $key->member_id ? Bean::load('member', (int)$key->member_id) : null;
        $createdBy = $key->created_by_member_id ? Bean::load('member', (int)$key->created_by_member_id) : null;
        ...
        return [
            ...
            'token' => $key->token,
            'token_preview' => $key->token ? substr($key->token, 0, 12) . '...' : '—',
            'scopes' => $scopes,
