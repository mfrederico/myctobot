...
?>
<div class="container py-4" data-assist-page="admin/members" data-assist-purpose="Manage workspace members, permissions, and invitations">
                            ...
                            <tr class="<?= $member->status === 'pending' ? 'table-warning' : '' ?>">
                                ...
                                <td>
                                    <div class="d-flex align-items-center">
                                        <?php if ($member->avatar_url): ?>
                                            <img src="<?= h($member->avatar_url) ?>" class="rounded-circle me-2" style="width: 32px; height: 32px;">
                                        <?php else: ?>
                                        ...
                                        <?php endif; ?>
                                        <div>
                                            <strong><?= h($member->display_name ?: $member->username ?: 'Unnamed') ?></strong>
                                            <br>
                                ...
                                <td>
                                    <?php if ($member->invited_by):
                                        $inviter = \app\Bean::load('member', (int)$member->invited_by);
                                        if ($inviter && $inviter->id): ?>
                                            <small title="Member #<?= $inviter->id ?>"><?= h($inviter->display_name ?: $inviter->username ?: $inviter->email) ?></small>
                                        <?php else: ?>
                                            <small class="text-muted">#<?= h($member->invited_by) ?></small>
                                        <?php endif; ?>
                                    <?php else: ?>
                                ...
                                <td>
                                    <?php if ($member->invite_sent_at): ?>
                                        <span title="<?= h($member->invite_sent_at) ?>">
                                ...
                                <td>
                                    <?php if ($member->last_login): ?>
                                        <span title="<?= h($member->last_login) ?>">
                                    ...
                                    <?php if (!$isSystemUser): ?>
                                        <?php if ($member->status === 'pending'): ?>
                                            <a href="/admin/resendinvite?id=<?= $member->id ?>"
                                               ...
                                               onclick="return confirm('Resend invitation to <?= h($member->email) ?>?')">
...
function exportMembers() {
    ...
    let csv = 'ID,Name,Email,Level,Status,Invited By,Invite Sent,Last Login\n';
    ...
    rows.forEach(row => {
        if (row.cells.length >= 7) {
            ...
            const invitedBy = row.cells[4].textContent.trim();
            const inviteSent = row.cells[5].textContent.trim();
            const lastLogin = row.cells[6].textContent.trim();

            const rowData = [id, name, email, level, status, invitedBy, inviteSent, lastLogin].map(cell =>
                '"' + cell.replace(/"/g, '""') + '"'
            );
            csv += rowData.join(',') + '\n';
        }
