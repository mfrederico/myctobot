                ...
                <div class="card-body text-center py-5">
                    ...
                    <div class="mb-4">
                        <span class="badge bg-<?= $status === 'completed' ? 'success' : ($status === 'running' ? 'primary' : ($status === 'awaiting_input' ? 'warning' : 'secondary')) ?> fs-6">
                            ...
                            <?php else: ?>
                            <i class="bi bi-clock"></i> <?= ucfirst(htmlspecialchars($status)) ?>
                            <?php endif; ?>
                        </span>
                    </div>
                    <?php endif; ?>
                    ...
                    <a href="/pipelines/form/<?= htmlspecialchars($run_uid) ?>" class="btn btn-primary btn-lg" id="returnBtn">
                        <i class="bi bi-arrow-left"></i> Return to Chat
                    </a>
                    <script>
                        // Auto-redirect back to form after 2 seconds
                        setTimeout(() => {
                            window.location.href = '/pipelines/form/<?= htmlspecialchars($run_uid) ?>';
                        }, 2000);
                    </script>
